...
class CategoryController extends Controller
{
    ...
    {
        // بارگذاری بازگشتی همه زیردسته‌ها با مرتب‌سازی
        $categories = Category::with(['children' => function($query) {
            $query->with(['children' => function($q) {
                $q->with(['children' => function($r) {
                    $r->with('children')->orderBy('sort_order');
                }])->orderBy('sort_order');
            }])->orderBy('sort_order');
        }])
        ->whereNull('parent_id')
        ...
        // اگر `CategoryResource` مشکل داره، مستقیم JSON برگردون
        return response()->json([
            'success' => true,
    ...
    private function formatMenu($categories)
    {
        ...
        foreach ($categories as $category) {
            ...
            // اگر فرزندان داره، بازگشتی فرمت کن
            if ($category->children && $category->children->isNotEmpty()) {
                $item['children'] = $this->formatMenu($category->children);
            }
