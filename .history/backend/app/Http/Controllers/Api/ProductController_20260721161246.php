...
class ProductController extends Controller
{
    ...
    // ================================================================
    // INDEX (Filter + Search + Sort + Pagination + Scroll)
    // ================================================================
    #[OA\Get(
        path: '/api/products',
        tags: ['Products'],
        summary: 'List products with filters',
        description: 'Get products with filtering, search, sorting and pagination',
        parameters: [
            new OA\Parameter(
                name: 'category_id',
                in: 'query',
            ...
            new OA\Parameter(
                ...
                schema: new OA\Schema(type: 'string', default: 'created_at', enum: ['price', 'view_count', 'created_at', 'title'])
            ),
            new OA\Parameter(
                name: 'sort_order',
                in: 'query',
    ...
    public function index(Request $request)
    {
        ...
        if ($request->has('category_id')) {
            ...
            $query->whereHas('categories', fn($q) => $q->whereIn('category_id', $allCategoryIds));
        }
        ...
        // ===== 3. Price filter =====
        if ($request->has('min_base_price') || $request->has('max_base_price')) {
        ...
        if ($request->has('attributes_id')) {
            ...
            $query->whereHas('variants.attributeValues', fn($q) => $q->whereIn('attribute_value_id', $attributeIds));
        }
        ...
        if ($sortBy === 'price') {
            ...
            $query->orderBy(
                ProductVariant::selectRaw("
                    MIN(
                        CASE
                            WHEN discounts.type = 'percent'
                            THEN product_variants.base_price - 
                                 (product_variants.base_price * discounts.value / 100)
                            WHEN discounts.type = 'fixed'
                            THEN product_variants.base_price - discounts.value
                            ELSE product_variants.base_price
                        END
                    )
                ")
                ->leftJoin(
                    'discountables',
                    'discountables.discountable_id',
                    '=',
                    'product_variants.id'
                )
                ->leftJoin(
        ...
        return $this->standardPaginate($query, $request);
    }

    // ================================================================
    // Standard Pagination
    // ================================================================
    private function standardPaginate($query, $request)
    {
    ...
    // ================================================================
    private function cursorPaginate($query, $request)
    {
