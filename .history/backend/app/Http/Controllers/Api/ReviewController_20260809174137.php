...
#[OA\Post(
    ...
    description: 'Toggle like/dislike on a review. If same type exists, it will be removed (toggle). If different type exists, it will be changed.',
    security: [['bearerAuth' => []]],
    parameters: [
        new OA\Parameter(
            name: 'review',
    ...
    responses: [
        new OA\Response(
            ...
            content: new OA\JsonContent(
                properties: [
                    new OA\Property(property: 'success', type: 'boolean', example: true),
                    new OA\Property(property: 'message', type: 'string', example: 'Reaction added successfully.'),
                    new OA\Property(property: 'user_reaction', type: 'string', nullable: true, example: 'like'),
                    new OA\Property(property: 'likes_count', type: 'integer', example: 12),
                    new OA\Property(property: 'dislikes_count', type: 'integer', example: 3),
                ]
            )
        ),
        new OA\Response(response: 401, description: 'Unauthenticated'),
...
public function react(ReviewReactionRequest $request, Review $review)
{
    ...
    $existingReaction = ReviewReaction::where('user_id', $user->id)
        ->where('review_id', $review->id)
        ->first();

    // اگر قبلاً همین واکنش رو زده → حذف (toggle)
    ...
    // اگر واکنش متفاوت زده → تغییر
    if ($existingReaction) {
        $existingReaction->update(['type' => $request->type]);

        return response()->json([
            'success' => true,
            'message' => 'Reaction changed to ' . $request->type . '.',
