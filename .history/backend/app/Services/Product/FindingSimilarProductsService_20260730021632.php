 ...
 * 
 * Finds similar/related products based on:
 * 1. Same Category + Same Brand (priority 1)
 * 2. Same Category only (priority 2)
 * 3. Popular products from same Brand (priority 3)
 */
class RelatedProductsService
{
     ...
     * 
     * @param Product $product - محصول مرجع
     * @param int $limit - حداکثر تعداد
     * @return Collection - لیست محصولات مشابه
     */
    public function find(Product $product, int $limit = 8): Collection
    {
        ...
        // 1️⃣ اولویت اول: هم‌دسته + هم‌برند
        ...
        // 2️⃣ اولویت دوم: اگه هنوز کم داریم، هم‌دسته
        ...
        // 3️⃣ اولویت سوم: اگه بازم کم داریم، محبوب برند
        if ($relatedProducts->count() < $limit) {
        ...
        // برگردون (حداکثر $limit تا)
        return $relatedProducts->take($limit)->values();
    }


    /**
     * 1️⃣ محصولات هم‌دسته و هم‌برند
     */
    private function getSameCategoryAndBrand(Product $product, int $limit): Collection
    {
        // اگه محصول برند نداره، Collection خالی برگردون
        if (!$product->brand_id) {
            return collect();
        }
        
        // ID های دسته‌های این محصول رو بگیر
        $categoryIds = $product->categories->pluck('id')->toArray();
        
        // اگه دسته‌ای نداره، خالی برگردون
