...
class ProductService
{
    ...
    /**
     * ✅ فیلتر دسته‌بندی - بهبودیافته
     * پشتیبانی از category_id و category_ids
     */
    private function applyCategoryFilter(Builder $query, Request $request): void
    {
        // 🎯 حالت ۱: چند دسته (category_ids=1,2,3)
        if ($request->filled('category_ids')) {
            $ids = array_filter(explode(',', $request->category_ids));
            
            if (empty($ids)) return;

            $allCategoryIds = $this->getAllCategoryIdsWithChildren($ids);
        ...
        // 🎯 حالت ۲: یک دسته (category_id=1)
        if ($request->filled('category_id')) {
            $categoryId = (int) $request->category_id;
            $allCategoryIds = $this->getAllCategoryIdsWithChildren([$categoryId]);
    ...
    /**
     * دریافت آی‌دی دسته‌ها به همراه تمام زیردسته‌های فعال
     */
    private function getAllCategoryIdsWithChildren(array $categoryIds): array
    {
        $allIds = [];
        ...
        foreach ($categoryIds as $id) {
            ...
            // زیردسته‌های فعال
            $subIds = Category::where('parent_id', $id)
    ...
    private function applyPriceFilter(Builder $query, Request $request): void
    {
        ...
        $query->whereHas('variants', function ($q) use ($request) {
            if ($request->has('min_price')) {
                $q->where('base_price', '>=', (float) $request->min_price);
            }
            if ($request->has('max_price')) {
                $q->where('base_price', '<=', (float) $request->max_price);
            }
        });
    }
    ...
    {
        if (!$request->filled('attributes_id')) return;
    ...
    /**
     * ✅ محاسبه قیمت نهایی، تخفیف و کمپین - نسخه واحد
     */
    private function attachPricingData(Product $product): Product
    {
        ...
        if (!$variant) {
            ...
            $product->_campaign_icon = null;
            $product->_campaign_color = null;
            $product->_campaign_ends_at = null;
            $product->_discount_percent = 0;
            $product->_final_price = 0;
            $product->_base_price = 0;
            $product->_discount_amount = 0;
            return $product;
        }
