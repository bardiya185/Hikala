...
{
    // 🔍 گرفتن همه زیردسته‌ها در یه Query (بهینه‌تر از foreach)
    $subCategoryIds = Category::whereIn('parent_id', $categoryIds)
        ->where('is_active', 1)
        ->pluck('id')
        ->toArray();

    // 🎯 ترکیب دسته‌های اصلی + زیردسته‌ها + حذف تکراری
    return array_unique(array_merge($categoryIds, $subCategoryIds));
}
