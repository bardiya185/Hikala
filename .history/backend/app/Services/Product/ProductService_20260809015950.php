...
private function applyCategoryFilter(Builder $query, Request $request): void
{
    ...
    if ($request->has('category_ids')) {
        ...
        if (empty($allCategoryIds)) return; // 🔥 اگه هیچ دسته فعالی نبود
    ...
    if ($request->has('category_id')) {
        ...
        if (empty($allCategoryIds)) return; // 🔥 اگه دسته غیرفعال بود
...
/**
 * گرفتن دسته‌ها + زیردسته‌های فعال (فقط اونایی که خودشون هم فعالن)
 */
private function getCategoryWithSubcategories(array $categoryIds): array
{
    // 🔥 اول چک کن که خود دسته‌های اصلی فعال هستن یا نه
    $activeParentIds = Category::whereIn('id', $categoryIds)
        ->where('is_active', 1)
        ->pluck('id')
        ->toArray();

    // 🚫 اگه هیچ دسته فعالی نبود، آرایه خالی برگردون
    if (empty($activeParentIds)) {
        return [];
    }

    // 🔍 گرفتن زیردسته‌های فعال
    $subCategoryIds = Category::whereIn('parent_id', $activeParentIds)
        ->where('is_active', 1)
        ->pluck('id')
        ->toArray();

    // 🎯 ترکیب + حذف تکراری
    return array_unique(array_merge($activeParentIds, $subCategoryIds));
}
