 ...
 * فقط دسته‌هایی که خودشون و همه‌ی پدرانشون فعال باشن
 */
...
/**
 * گرفتن دسته‌ها + زیردسته‌های فعال
 * فقط دسته‌هایی که خودشون و همه‌ی پدرانشون فعال باشن
 */
private function getCategoryWithSubcategories(array $categoryIds): array
{
    // 🔥 اول چک کن دسته‌های اصلی + پدرانشون فعال باشن
    $activeParentIds = [];
    foreach ($categoryIds as $id) {
        if ($this->isCategoryAndAncestorsActive((int) $id)) {
            $activeParentIds[] = (int) $id;
        }
    }
    ...
    // 🔍 گرفتن همه زیردسته‌های فعال (به صورت بازگشتی)
    $allIds = $activeParentIds;
    $currentIds = $activeParentIds;
...
/**
 * 🔥 چک کن دسته و همه‌ی پدرانش فعال باشن
 */
