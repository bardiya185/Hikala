...
private function applyCategoryFilter(Builder $query, Request $request): void
{
    ...
    if ($request->has('category_ids')) {
        ...
        $allCategoryIds = $this->getValidCategoryIds($ids);
    ...
    if ($request->has('category_id')) {
        $allCategoryIds = $this->getValidCategoryIds([$request->category_id]);
...
/**
 * گرفتن دسته‌های معتبر + زیردسته‌های فعال
 * فقط دسته‌ای که خودش و همه‌ی پدرانش فعال باشن
 */
private function getValidCategoryIds(array $categoryIds): array
{
    // 🔥 اول: همه‌ی دسته‌های فعال رو یکجا load کن (فقط ۱ Query!)
    $allActiveCategories = Category::where('is_active', 1)
        ->pluck('parent_id', 'id')
        ->toArray();
    // ساختار: [category_id => parent_id]

    // 🔍 دسته‌های معتبر (خودش + همه پدرانش فعال)
    $validIds = [];
    foreach ($categoryIds as $id) {
        $id = (int) $id;
        if ($this->checkChain($id, $allActiveCategories)) {
            $validIds[] = $id;
        }
    }
    ...
    // 🔍 گرفتن همه‌ی زیردسته‌های فعال (Recursive)
    $allIds = $validIds;
    $currentIds = $validIds;

    while (!empty($currentIds)) {
        $subIds = [];
        foreach ($allActiveCategories as $catId => $parentId) {
            if (in_array($parentId, $currentIds) && !in_array($catId, $allIds)) {
                $subIds[] = $catId;
            }
        }
...
/**
 * 🔥 چک کن دسته و همه‌ی پدرانش در لیست فعال‌ها هستن
 */
private function checkChain(int $categoryId, array $activeMap): bool
{
    // اگه در لیست فعال‌ها نیست، غیرفعاله
    if (!array_key_exists($categoryId, $activeMap)) {
        return false;
    }

    $parentId = $activeMap[$categoryId];

    // اگه parent نداره (root)، فعاله ✅
    if ($parentId === null) {
        return true;
    }

    // اگه parent داره، برو بالا و چک کن
    return $this->checkChain((int) $parentId, $activeMap);
}
private function isCategoryAndAncestorsActive(int $categoryId): bool
{
    $category = Category::find($categoryId);

    // اگه دسته موجود نیست یا غیرفعاله
    if (!$category || !$category->is_active) {
        return false;
    }

    // اگه پدر نداره (root category)، فعاله ✅
    if (!$category->parent_id) {
        return true;
    }

    // 🔄 چک کن پدر و پدرِ پدر و ...
    return $this->isCategoryAndAncestorsActive($category->parent_id);
}
