 ...
 * فقط اگه خود دسته و همه پدرانش فعال باشن
 */
private function getCategoryWithSubcategories(array $categoryIds): array
{
    ...
    // 🔍 گرفتن زیردسته‌های فعال (فقط یک سطح پایین‌تر)
    $subCategoryIds = Category::whereIn('parent_id', $validIds)
        ->where('is_active', 1)
        ->pluck('id')
        ->toArray();

    return array_unique(array_merge($validIds, $subCategoryIds));
}

/**
 * چک کن دسته و همه پدرانش فعال باشن
 */
private function isChainActive(int $categoryId): bool
{
    $current = Category::find($categoryId);

    // اگه دسته نیست یا غیرفعاله
