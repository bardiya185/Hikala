...
class ProductDatabaseSeeder extends Seeder
{
    public function run()
    {
        ...
        $productList = [
            ...
            ['title' => 'Motorola Edge 40 256GB', 'brand' => 'Motorola', 'category' => 'motorola', 'price' => 600, 'rating' => 4.3],
            ['title' => 'Nothing Phone 2 256GB', 'brand' => 'Nothing', 'category' => 'nothing-phone', 'price' => 650, 'rating' => 4.4],
            ['title' => 'Realme GT 3 256GB', 'brand' => 'Realme', 'category' => 'realme', 'price' => 550, 'rating' => 4.3],
            ...
            ['title' => 'Dior Sauvage 100ml', 'brand' => 'Dior', 'category' => 'dior-sauvage', 'price' => 100, 'rating' => 4.6],
            ['title' => 'Chanel No.5 50ml', 'brand' => 'Chanel', 'category' => 'chanel-no-5', 'price' => 150, 'rating' => 4.7],
            ...
            ['title' => 'DeWalt Angle Grinder', 'brand' => 'DeWalt', 'category' => 'dewalt-grinder', 'price' => 120, 'rating' => 4.6],

            // ===== BOOKS =====
            ['title' => '1984 George Orwell', 'brand' => 'Apple', 'category' => '1984-george-orwell', 'price' => 25, 'rating' => 4.8],
            ['title' => 'Atomic Habits', 'brand' => 'Apple', 'category' => 'atomic-habits', 'price' => 30, 'rating' => 4.9],
        ...
        $productCount = 0;
        $variantCount = 0;
        $colors = ['Black', 'White', 'Silver', 'Gold', 'Blue', 'Red', 'Purple', 'Space Gray', 'Midnight', 'Starlight'];
        ...
        foreach ($productList as $productData) {
            ...
            $salePrice = round($salePrice / 10) * 10;

            $description = "Premium " . $title . " with high-quality features.";
            ...
            // ================================================================
            // 4. CREATE VARIANTS با حداقل ۱۰ ویژگی
            // ================================================================
            ...
            for ($v = 1; $v <= $variantCountPerProduct; $v++) {
                ...
                // ================================================================
                // ✅ ATTACH 10+ ATTRIBUTES TO VARIANT
                // ================================================================
    ...
    // ================================================================
    // متد اختصاصی برای اتصال حداقل ۱۰ ویژگی به هر تنوع
    // ================================================================
    private function attachManyAttributes($variantId, $color, $storage, $categoryId)
    {
        // 1. دریافت همه ویژگی‌های مربوط به این دسته‌بندی
        ...
        if (empty($categoryAttrs)) {
            // اگر دسته‌بندی ویژگی نداشت، از ویژگی‌های عمومی استفاده کن
        ...
        // 2. حداقل ۱۰ ویژگی انتخاب کن
        $minAttrs = 10;
        $selectedAttrIds = [];

        // اول ویژگی‌های اجباری (color و storage)
        ...
        if ($storageAttr) {
            ...
            if ($storageValue) {
                ...
                $selectedAttrIds[] = $storageAttr->id;
            }
        }

        // 3. ویژگی‌های تصادفی اضافی (حداقل ۸ تا دیگه)
        $remainingAttrs = array_diff($categoryAttrs, $selectedAttrIds);
        $remainingAttrs = array_values($remainingAttrs);

        shuffle($remainingAttrs);
        $needed = max($minAttrs - count($selectedAttrIds), 8);
        $selectedRandomAttrs = array_slice($remainingAttrs, 0, $needed);

        foreach ($selectedRandomAttrs as $attrId) {
