...
class ProductDatabaseSeeder extends Seeder
{
    ...
    // ================================================================
    // ✅ متد attach ویژگی‌ها به variant
    // یک رنگ + دقیقاً 10 ویژگی دیگر
    // ================================================================
    private function attachVariantAttributes(int $variantId, string $colorName, int $categoryId): void
    {
        // ---- 1. فقط یک رنگ ----
        ...
        if ($colorAttr) {
            ...
            if ($colorValue) {
                DB::table('product_variant_attribute_values')->insertOrIgnore([
        ...
        // ---- 2. پیدا کردن attribute‌های دسته‌بندی (به جز color) ----
        $categoryAttrIds = DB::table('category_attributes')
            ->where('category_id', $categoryId)
            ->pluck('attribute_id')
        ...
        // اگر attribute کافی نبود، از attribute‌های عمومی هم استفاده کن
        if (count($categoryAttrIds) < 10) {
            $generalAttrSlugs = ['storage', 'ram', 'processor', 'battery', 'material',
                                  'weight', 'display-size', 'display-type', 'connectivity',
                                  'battery-life', 'year', 'waterproof'];

            $generalAttrIds = DB::table('attributes')
                ->whereIn('slug', $generalAttrSlugs)
                ->pluck('id')
                ->reject(fn($id) => $colorAttr && $id == $colorAttr->id)
                ->reject(fn($id) => in_array($id, $categoryAttrIds))
                ->values()
                ->toArray();

            $categoryAttrIds = array_merge($categoryAttrIds, $generalAttrIds);
        }

        // ---- 3. انتخاب دقیقاً 10 attribute ----
        shuffle($categoryAttrIds);
        $selectedAttrIds = array_slice($categoryAttrIds, 0, 10);

        // اگر هنوز کمتر از 10 تا داریم، تا 10 تا تکرار کن
        while (count($selectedAttrIds) < 10) {
            $selectedAttrIds = array_merge($selectedAttrIds, $categoryAttrIds);
        }
        $selectedAttrIds = array_slice($selectedAttrIds, 0, 10);

        // ---- 4. برای هر attribute یک value تصادفی انتخاب و ذخیره کن ----
        $usedAttrIds = []; // جلوگیری از attribute تکراری

        foreach ($selectedAttrIds as $attrId) {
            // هر attribute فقط یک بار
            if (in_array($attrId, $usedAttrIds)) {
                continue;
            }
            $usedAttrIds[] = $attrId;
