...
function ProductsDe() {
  ...
  useEffect(() => {
    // 1. ابتدا کاملاً مخفی کنید
    gsap.set(".animate-split", { 
      opacity: 0,
      visibility: "hidden" 
    });
  
    // 2. SplitText رو اعمال کنید
    const split = new SplitText(".animate-split", { 
      type: "words",
      wordsClass: "word" // این کلاس به هر کلمه اضافه میشه
    });
  
    // 3. همه کلمات رو مخفی کنید
    ...
    // 4. با تاخیر 1 ثانیه نمایش بدید
    setTimeout(() => {
      // اول تگ اصلی رو نمایش بدید
      gsap.to(".animate-split", {
        opacity: 1,
        visibility: "visible",
        duration: 0.1,
        onComplete: () => {
          // بعد کلمات رو یکی یکی نمایش بدید
          gsap.to(split.words, {
            opacity: 1,
            y: 0,
            scale: 1,
