...
function ProductsDe() {
  ...
  // استفاده از ref برای هر عنصر
  const text1Ref = useRef(null);
  const text2Ref = useRef(null);

  useEffect(() => {
    // انیمیشن برای متن اول
    const element1 = text1Ref.current;
    if (element1) {
      gsap.set(element1, { opacity: 0, visibility: "hidden" });
      
      const split1 = new SplitText(element1, { 
        type: "words",
        wordsClass: "word" 
      });
      
      gsap.set(split1.words, { opacity: 0, y: 30, scale: 0.8 });
    ...
    // انیمیشن برای متن دوم (با تاخیر بیشتر)
    const element2 = text2Ref.current;
    if (element2) {
      gsap.set(element2, { opacity: 0, visibility: "hidden" });
      
      const split2 = new SplitText(element2, { 
        type: "words",
        wordsClass: "word" 
      });
      
      gsap.set(split2.words, { opacity: 0, y: 30, scale: 0.8 });
      ...
      <div className="flex gap-3">
        ...
        <div className="w-[360px] h-[500px] border border-solid border-neutral-500 rounded-lg mt-16">
          <div className="flex justify-between px-5 pt-5">
            <p>Seller</p>
            {/* استفاده از ref برای متن اول */}
            <span ref={text1Ref} className="text-orange-400 font-medium">
              3 other sellers
            </span>
          </div>
          ...
          <div className="pl-5 mt-3 flex items-center gap-2">
            ...
            {/* استفاده از ref برای متن دوم */}
            <span ref={text2Ref} className="text-orange-400 font-medium">
