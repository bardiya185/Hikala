...
function Products({ data, current_sort, current_sortorder }) {
  ...
  // گرفتن تصویر از محصول
  const getProductImage = (product) => {
    // اگر images وجود داره و اولین تصویر رو داره
    if (product.images && product.images.length > 0) {
      return product.images[0].image_url;
    }
    // اگر image_path وجود داره (برای سازگاری)
    if (product.image_path) {
      return product.image_path;
    }
    // placeholder
    return `https://via.placeholder.com/300x300?text=${encodeURIComponent(product.title || 'Product')}`;
  };
              ...
              return (
                ...
                >
                  <div className="rounded-[10px] w-full h-full border border-solid border-neutral-100 p-4 flex flex-col justify-between">
                    <div>
                      <div className="w-full overflow-hidden rounded-[20px] aspect-[4/5] relative flex items-center justify-center bg-gray-50">
                        <img
                          src={imageUrl}
                          ...
                          onError={(e) => {
                            // اگه عکس لود نشد، placeholder
                            e.target.src = `https://via.placeholder.com/300x300?text=${encodeURIComponent(product?.title || 'Product')}`;
                          }}
                        />
                      </div>
