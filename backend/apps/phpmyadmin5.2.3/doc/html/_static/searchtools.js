...
const Search = {
    ...
    query: (query) => {
            ...
            if (
                ...
                for (const [file, id] of foundTitles) {
                    let score = Math.round(
                        (100 * queryLower.length) / title.length,
                    );
                    results.push([
                        docNames[file],
                        titles[file] !== title
        ...
        for (const [entry, foundEntries] of Object.entries(indexEntries)) {
            if (
                entry.includes(queryLower) &&
                queryLower.length >= entry.length / 2
            ) {
                for (const [file, id] of foundEntries) {
                    let score = Math.round(
                        (100 * queryLower.length) / entry.length,
                    );
                    results.push([
                        docNames[file],
                        titles[file],
        ...
        if (Scorer.score)
            results.forEach((item) => (item[4] = Scorer.score(item)));
        results.sort((a, b) => {
            const leftScore = a[4];
    ...
    performObjectSearch: (object, objectTerms) => {
        ...
        const objectSearchCallback = (prefix, match) => {
            ...
            if (otherTerms.size > 0) {
                const haystack =
                    `${prefix} ${name} ${objName} ${title}`.toLowerCase();
                if (
                    [...otherTerms].some(
                        (otherTerm) => haystack.indexOf(otherTerm) < 0,
                    )
                )
                    return;
            }
        ...
        Object.keys(objects).forEach((prefix) => {
            if (!(objects[prefix] instanceof Array)) {
                objects[prefix] = Object.entries(objects[prefix]).map(
    ...
    performTermsSearch: (searchTerms, excludedTerms) => {
        ...
        searchTerms.forEach((word) => {
            ...
            arr.forEach((record) => {
                ...
                if (recordFiles.length === undefined)
        ...
        for (const [file, wordList] of fileMap) {
            ...
            const score = Math.max(
                ...wordList.map((w) => scoreMap.get(file)[w]),
            );
            results.push([
                docNames[file],
                titles[file],
