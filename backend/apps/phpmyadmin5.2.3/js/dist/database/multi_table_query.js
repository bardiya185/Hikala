...
AJAX.registerOnload("database/multi_table_query.js", function () {
    ...
    $("#update_query_button").on("click", function () {
        ...
        $(".tableNameSelect").each(function () {
            ...
            if ($(this).val() !== "" && $show.prop("checked")) {
                ...
                if (tableAlias !== "") {
                    columns.push([
                        tableAlias,
                        $(this).siblings(".columnNameSelect").first().val(),
                    ]);
                } else {
                    columns.push([
                        $(this).val(),
        ...
        var query =
            "SELECT " + "`" + Functions.escapeBacktick(columns[0][0]) + "`.";
        if (columns[0][1] === "*") {
            query += "*";
        } else {
        ...
        for (var i = 1; i < columns.length; i++) {
            ...
            if (columns[i][2] !== "") {
                query +=
                    " AS `" + Functions.escapeBacktick(columns[i][2]) + "`";
            }
        }
        query += "\nFROM ";
    ...
    $("#submit_query").on("click", function () {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $resultsDom
                    .find(
                        ".autosubmit, .pageselector, .showAllRows, .filter_rows",
                    )
                    .each(function () {
                        $(this).on(
                            "change click select focus",
                            function (event) {
                                event.preventDefault();
                            },
                        );
    ...
    $("#add_column_button").on("click", function () {
        ...
        $newColumnDom
            .find(".jsCriteriaButton")
            .first()
            .attr(
                "data-bs-target",
                "#criteriaOptionsExtra" + columnCount.toString(),
            );
        $newColumnDom
            .find(".jsCriteriaButton")
            .first()
        ...
        $newColumnDom
            .find(".jsCriteriaOptions")
            .first()
            .attr("id", "criteriaOptionsExtra" + columnCount.toString());
    ...
    function showHint(opSelect) {
        ...
        const criteriaInputCol = opSelect
            .closest("table")
            .find(".rhs_text_val")
            .parent();
        ...
        Object.keys(hints).includes(value) &&
            criteriaInputCol.append(
                '<p class="rhs_hint">'.concat(hints[value], "</p>"),
            );
    }
