...
function generateCondition(criteriaDiv, table) {
    ...
    let query =
        "`" +
        Functions.escapeBacktick(tableAlias === "" ? tableName : tableAlias) +
        "`.";
    query +=
        "`" +
        Functions.escapeBacktick(
            table.parent().find(".opColumn").first().val(),
        ) +
        "`";
    if (criteriaDiv.find(".criteria_rhs").first().val() === "text") {
        ...
        query +=
            " `" +
            Functions.escapeBacktick(
                criteriaDiv.find(".tableNameSelect").first().val(),
            ) +
            "`.";
        query +=
            "`" +
            Functions.escapeBacktick(
                criteriaDiv.find(".opColumn").first().val(),
            ) +
            "`";
    }
    return query;
...
function generateJoin(newTable, tableAliases, fk) {
    ...
    if (tableAliases[fk.TABLE_NAME][0] !== "") {
        query +=
            " AS `" + Functions.escapeBacktick(tableAliases[newTable][0]) + "`";
        query +=
            " ON `" +
            Functions.escapeBacktick(tableAliases[fk.TABLE_NAME][0]) +
            "`";
    } else {
    ...
    if (tableAliases[fk.REFERENCED_TABLE_NAME][0] !== "") {
        query +=
            " = `" +
            Functions.escapeBacktick(
                tableAliases[fk.REFERENCED_TABLE_NAME][0],
            ) +
            "`";
    } else {
        query +=
            " = `" + Functions.escapeBacktick(fk.REFERENCED_TABLE_NAME) + "`";
    }
...
function existReference(table, fk, usedTables) {
    var isReferredBy =
        fk.TABLE_NAME === table &&
        usedTables.includes(fk.REFERENCED_TABLE_NAME);
    var isReferencedBy =
        fk.REFERENCED_TABLE_NAME === table &&
        usedTables.includes(fk.TABLE_NAME);
...
function appendTable(table, tableAliases, usedTables, foreignKeys) {
    ...
    if (query === "") {
        ...
        if (tableAliases[table][0] !== "") {
            query +=
                " AS `" +
                Functions.escapeBacktick(tableAliases[table][0]) +
                "`";
        }
    }
