...
var ErrorReport = {
    ...
    errorHandler: function (exception) {
        // issue: 14359
        if (
            JSON.stringify(ErrorReport.lastException) ===
    ...
    showReportDialog: function (exception) {
        ...
        $.post("index.php?route=/error-report", reportData).done(
            function (data) {
    ...
    showErrorNotification: function () {
        ...
        var $div = $(
            '<div class="alert alert-danger" role="alert" id="error_notification_' +
                key +
                '"></div>',
        ).append(Functions.getImage("s_error") + Messages.strErrorOccurred);
        var $buttons = $('<div class="float-end"></div>');
        var buttonHtml =
            '<button class="btn btn-primary" id="show_error_report_' +
            key +
            '">';
        buttonHtml += Messages.strShowReportDetails;
        buttonHtml += "</button>";
        var settingsUrl =
            "index.php?route=/preferences/features&server=" +
            CommonParams.get("server");
        buttonHtml += '<a class="ajax" href="' + settingsUrl + '">';
        buttonHtml += Functions.getImage(
            "s_cog",
            Messages.strChangeReportSettings,
        );
        buttonHtml += "</a>";
        buttonHtml +=
            '<a href="#" id="ignore_error_' +
            key +
        ...
        $(document).on(
            "click",
            "#show_error_report_" + key,
            ErrorReport.createReportDialog,
        );
        $(document).on(
            "click",
            "#ignore_error_" + key,
    ...
    removeErrorNotification: function (e) {
        ...
        $("#error_notification_" + $(this).data("notification-id")).fadeOut(
            function () {
                $(this).remove();
            },
        );
    ...
    extractExceptionName: function (exception) {
        if (
            ...
            typeof exception.message === "undefined"
        ) {
            return "";
        }
        var reg = /([a-zA-Z]+):/;
                ...
                if (stack.context && stack.context.length) {
                    for (var j = 0; j < stack.context.length; j++) {
                        if (stack.context[j].length > 80) {
                            stack.context[j] =
                                stack.context[j].substring(-1, 75) + "//...";
                        }
                    }
                }
            }
    ...
    wrapFunction: function (func) {
        if (!func.wrapped) {
            ...
            newFunc.guid = func.guid =
                func.guid || newFunc.guid || jQuery.guid++;
            return newFunc;
        } else {
            return func;
        }
