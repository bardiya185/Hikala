...
var makeGrid = function (
    ...
    var isGridEditEnabled =
        enableGridEdit === undefined ? true : enableGridEdit;
    var g = {
        /** *********
         * Constant
         ***********/
        ...
        reposRsz: function () {
            ...
            var $firstRowCols = $(g.t)
                .find("tr")
                .first()
                .find("th.draggable:visible");
            var $resizeHandles = $(g.cRsz).find("div").removeClass("condition");
            $(g.t)
                .find("table.pma_table")
                .find("thead th")
                .first()
                .removeClass("before-condition");
            for (var n = 0, l = $firstRowCols.length; n < l; n++) {
                ...
                $($resizeHandles[n])
                    .css("left", $col.position().left + colWidth)
                    .show();
        ...
        sendColPrefs: function () {
            if ($(g.t).is(".ajax")) {
                ...
                $.post(
                    "index.php?route=/sql/set-column-preferences",
                    postParams,
                    function (data) {
        ...
        afterToggleCol: function () {
            ...
            g.visibleHeadersCount = $(g.t)
                .find("tr")
                .first()
                .find("th.draggable:visible").length;
            g.refreshRestoreButton();
        ...
        showEditCell: function (cell) {
            ...
            if ($(cell).is(".grid_edit") && !g.colRsz && !g.colReorder) {
                if (!g.isCellEditActive) {
                    ...
                    if (
                        $cell.attr("data-type") === "json" &&
                        $cell.is(".truncated") === false
                    ) {
            ...
            if (data) {
                if (g.currentEditCell) {
                    ...
                    if (isNull) {
                        ...
                        if (
                            CommonParams.get("pftext") === "P" &&
                            value.length > g.maxTruncatedLen
                        ) {
                            $thisField.addClass("truncated");
                            value =
                                value.substring(0, g.maxTruncatedLen) + "...";
                        }
                        ...
                        var decimals = parseInt(
                            $thisField.attr("data-decimals"),
                        );

                        // remove decimal places if column type not supported
                        if (
                            decimals === 0 &&
                            $thisField.attr("data-type").indexOf("time") !== -1
                        ) {
                        ...
                        if (
                            ...
                            newHtml = newHtml.substring(
                                0,
                                newHtml.length - (6 - decimals),
                            );
                        }
                        var selector = "span";
                        if (
                            $thisField.hasClass("hex") &&
                            $thisField.find("a").length
                        ) {
                            selector = "a";
                        }
        ...
        showEditArea: function () {
            if (!g.isCellEditActive) {
                ...
                if ($td.is(":not(.not_null)")) {
                    ...
                    $editArea.append(
                        '<div class="null_div"><label>NULL:<input type="checkbox"></label></div>',
                    );
                ...
                if ($td.is(".relation")) {
                    ...
                    postParams = {
                        ...
                        relation_key_or_display_column:
                            relationKeyOrDisplayColumn,
                    };
                    g.lastXHR = $.post(
                        "index.php?route=/sql/get-relational-values",
                        postParams,
                        function (data) {
                            g.lastXHR = null;
                    ...
                    g.lastXHR = $.post(
                        "index.php?route=/sql/get-enum-values",
                        postParams,
                        function (data) {
                            g.lastXHR = null;
                            if (
                    ...
                    g.lastXHR = $.post(
                        "index.php?route=/sql/get-set-values",
                        postParams,
                        function (data) {
                            g.lastXHR = null;
                            if (
                            ...
                            function (data) {
                                ...
                                if (
                                    ...
                                    if ($td.attr("data-type") === "json") {
                                        data.value = Functions.stringifyJSON(
                                            data.value,
                                            null,
                                            4,
                                        );
                    ...
                    $td.is(
                        ".timefield, .datefield, .datetimefield, .timestampfield",
                    )
                ) {
                    ...
                    if (
                        $td.attr("data-decimals") > 0 &&
                        $td.attr("data-type").indexOf("time") !== -1
                    ) {
                        if (
                            datetimeValue &&
                            datetimeValue.indexOf(".") === false
                        ) {
                            datetimeValue += ".";
                        }
                        if ($td.attr("data-decimals") > 3) {
                            ...
                            if (datetimeValue) {
                                ...
                                datetimeValue = datetimeValue.substring(
                                    0,
                                    datetimeValue.indexOf(".") + 7,
                                );
                    ...
                    $inputField.on("keyup", function (e) {
                        if (e.which === 13) {
                            ...
                            Functions.toggleDatepickerIfInvalid(
                                $td,
                                $inputField,
                            );
                        }
        ...
        postEditedCell: function (options) {
             ...
             */
            var relationalDisplay = $(g.o)
                .find("input[name=relational_display]:checked")
                .val();
            /**
             ...
             */
            var isUnique = $(g.t).find("td.edit_row_anchor").is(".nonunique")
                ? 0
                : 1;
            /**
            ...
            $(g.t)
                ...
                .each(function () {
                    ...
                    var conditionArrayContent = $tr
                        .find(".condition_array")
                        .val();
                    ...
                    $tr.find(".to_be_saved").each(function () {
                        ...
                        if (isNull) {
                            ...
                            if ($thisField.attr("data-type") !== "json") {
                                ...
                                const JSONString = Functions.stringifyJSON(
                                    $thisField.data("value"),
                                );
                        ...
                        if (
                            whereClause.indexOf(Sql.urlEncode(fieldName)) > -1
                        ) {
                            var fieldStr =
                                "`" + g.table + "`." + "`" + fieldName + "`";
                            for (var field in conditionArray) {
                                if (field.indexOf(fieldStr) > -1) {
                                    conditionArray[field] = isNull
                                        ? "IS NULL"
                                        : "= '" +
                                          thisFieldParams[fieldName].replace(
                                              /'/g,
                                              "''",
                                          ) +
                                          "'";
            ...
            if (!g.saveCellsAtOnce) {
                ...
                $(g.o)
                    .find("div.save_edited")
                    .addClass("saving_edited_data")
                    .find("input")
                    .prop("disabled", true); // disable the save button
            }
            $.ajax({
                type: "POST",
                ...
                success: function (data) {
                    ...
                    if (!g.saveCellsAtOnce) {
                        ...
                        $(g.cEdit)
                            .find(".edit_box")
                            .removeClass("edit_box_posting");
                    } else {
                        $(g.o)
                            .find("div.save_edited")
                            .removeClass("saving_edited_data")
                            .find("input")
                            .prop("disabled", false); // enable the save button back
                    }
                    if (typeof data !== "undefined" && data.success === true) {
                        ...
                        if (typeof data.sql_query !== "undefined") {
                            ...
                            var sqlOuter = $resultQuery
                                .find(".sqlOuter")
                                .wrap("<p>")
                                .parent()
                                .html();
                            var tools = $resultQuery
                                .find(".tools")
                                .wrap("<p>")
                                .parent()
                                .html();
                            ...
                            if (
                                typeof sqlOuter !== "undefined" &&
                                typeof tools !== "undefined"
                            ) {
                                $(g.o)
                                    .find(".result_query")
                                    .not($(g.o).find(".result_query").last())
                                    .remove();
                                var $existingQuery = $(g.o).find(
                                    ".result_query",
                                );
                                // If two query box exists update query in second else add a second box
                                if (
                                    $existingQuery.find("div.sqlOuter").length >
                                    1
                                ) {
        ...
        saveEditedCell: function () {
            ...
            if ($(g.cEdit).find(".edit_area").is(".edit_area_loading")) {
                ...
                if ($thisField.is(".bit")) {
                    thisFieldParams[fieldName] = $(g.cEdit)
                        .find(".edit_box")
                        .val();
                } else if ($thisField.is(".set")) {
                    $testElement = $(g.cEdit).find("select");
                    thisFieldParams[fieldName] = $testElement
                        .map(function () {
                            return $(this).val();
                        })
                        .get()
                    ...
                    if (
                        ...
                        thisFieldParams[fieldName] = Functions.getCellValue(
                            g.currentEditCell,
                        );
                    }
                } else {
        ...
        initColReorder: function () {
            ...
            $(g.t)
                ...
                .on("dblclick", function (e) {
                    ...
                    if (res) {
                        Functions.ajaxShowMessage(
                            Messages.strCopyColumnSuccess,
                            false,
                            "success",
                        );
                    } else {
                        Functions.ajaxShowMessage(
                            Messages.strCopyColumnFailure,
                            false,
                            "error",
                        );
                    }
        ...
        initGridEdit: function () {
            ...
            function handleCtrlNavigation(e) {
                if (
                    (e.ctrlKey && e.which === 38) ||
                    (e.altKey && e.which === 38)
                ) {
                    g.moveUp(e);
                } else if (
                    (e.ctrlKey && e.which === 40) ||
                    (e.altKey && e.which === 40)
                ) {
                    g.moveDown(e);
                } else if (
                    (e.ctrlKey && e.which === 37) ||
                    (e.altKey && e.which === 37)
                ) {
                    g.moveLeft(e);
                } else if (
                    (e.ctrlKey && e.which === 39) ||
                    (e.altKey && e.which === 39)
                ) {
                    g.moveRight(e);
                }
            ...
            $(g.cEditStd).html(
                '<input class="edit_box" rows="1"><div class="edit_area"></div>',
            );
            $(g.cEditStd).hide();
            ...
            $(g.cEditTextarea).html(
                '<textarea class="edit_box" rows="1"></textarea><div class="edit_area"></div>',
            );
            ...
            $("html")
                .on("click", function (e) {
                    ...
                    if (
                        $(e.target).parents().index($(g.cEdit)) === -1 &&
    ...
    $(t)
        .find("td.data.truncated:not(:has(>span))")
        .filter(function () {
        ...
        .wrapInner(function () {
            return (
                '<span title="' +
                Messages.strOriginalLength +
                " " +
                $(this).data("originallength") +
                '"></span>'
            );
        });
