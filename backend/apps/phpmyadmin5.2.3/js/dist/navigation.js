...
Navigation.treeStateUpdate = function () {
    ...
    if (isStorageSupported("sessionStorage")) {
        ...
        try {
            storage.setItem(
                "navTreePaths",
                JSON.stringify(Navigation.traverseForPaths()),
            );
...
Navigation.filterStateUpdate = function (filterName, filterValue) {
    if (isStorageSupported("sessionStorage")) {
        var storage = window.sessionStorage;
        try {
            var currentFilter = $.extend(
                {},
                JSON.parse(storage.getItem("navTreeSearchFilters")),
            );
            var filter = {};
            filter[filterName] = filterValue;
            currentFilter = $.extend(currentFilter, filter);
            storage.setItem(
                "navTreeSearchFilters",
...
Navigation.filterStateRestore = function () {
    if (
        ...
        var searchClauses = JSON.parse(
            window.sessionStorage.navTreeSearchFilters,
        );
        ...
        if (
            searchClauses.hasOwnProperty("dbFilter") &&
        ...
        $tableFilters.each(function () {
            ...
            if (
                searchClauses.hasOwnProperty(filterName) &&
...
Navigation.loadChildNodes = function (isNode, $expandElem, callback) {
    ...
    if (isNode) {
        ...
        if ($expandElem.closest("ul").hasClass("search_results")) {
            ...
            params.searchClause2 =
                Navigation.FastFilter.getSearchClause2($expandElem);
        }
    } else {
    ...
    $.post(
        "index.php?route=/navigation&ajax_request=1",
        params,
        function (data) {
...
Navigation.traverseForPaths = function () {
    ...
    var count = 0;
    $("#pma_navigation_tree")
        .find("a.expander:visible")
        .each(function () {
            if (
                $(this).find("img").is(".ic_b_minus") &&
                $(this).closest("li").find("div.list_container .ic_b_minus")
                    .length === 0
            ) {
                ...
                params["n" + count + "_pos2_value"] =
                    pos2Nav.attr("data-value");
                var pos3Nav = $(this).find("span.pos3_nav");
                params["n" + count + "_pos3_name"] = pos3Nav.attr("data-name");
                params["n" + count + "_pos3_value"] =
                    pos3Nav.attr("data-value");
                count++;
            }
        });
    return params;
...
$(function () {
    ...
    $(document).on(
        "click",
        "#pma_navigation_tree a.expander",
        function (event) {
            event.preventDefault();
     ...
     */
    $("#pma_navigation_tree").on(
        "click",
        "li.fast_filter button.searchClauseClear",
        Navigation.FastFilter.events.clear,
    );
    $("#pma_navigation_tree").on(
        "focus",
        "li.fast_filter input.searchClause",
        Navigation.FastFilter.events.focus,
    );
    $("#pma_navigation_tree").on(
        "blur",
        "li.fast_filter input.searchClause",
        Navigation.FastFilter.events.blur,
    );
    $("#pma_navigation_tree").on(
        "keyup",
        "li.fast_filter input.searchClause",
        Navigation.FastFilter.events.keyup,
    );
    ...
    $(document).on("click", ".favorite_table_anchor", function (event) {
        ...
        if ($self.data("favtargetn") !== null) {
            var $dataFavTargets = $(
                'a[data-favtargets="' + $self.data("favtargetn") + '"]',
            );
        ...
        $.ajax({
            ...
            type: "POST",
            data: {
                favoriteTables: hasLocalStorage
...
Navigation.showCurrent = function () {
    ...
    if (db) {
        $dbItem = findLoadedItem(
            $("#pma_navigation_tree").find("> div"),
            db,
            "database",
            !table,
        );
        if (
            $("#navi_db_select").length &&
            ...
            if (
                $("#pma_navigation_tree_content")
                    .find("span.loaded_db")
                    .first()
                    .text() !== $("#navi_db_select").val()
            ) {
                Navigation.loadChildNodes(
                    false,
                    $("option:selected", $("#navi_db_select")),
                    function () {
                        handleTableOrDb(
                            table,
                            $("#pma_navigation_tree_content"),
        ...
        autoexpand &&
        $("#pma_navigation_tree_content > ul > li.database").length === 1
    ) {
        // automatically expand the list if there is only single database

        // find the name of the database
        var dbItemName = "";
        $("#pma_navigation_tree_content > ul > li.database")
            .children("a")
            .each(function () {
                var name = $(this).text();
    ...
    function loadAndHighlightTableOrView($dbItem, itemName) {
        var $container = $dbItem.children("div.list_container");
        var $expander;
        var $whichItem = isItemInContainer(
            $container,
            itemName,
            "li.nav_node_table, li.view",
        );
...
Navigation.treePagination = function ($this) {
    ...
    if (searchClause) {
        params +=
            CommonParams.get("arg_separator") +
            "searchClause=" +
            encodeURIComponent(searchClause);
    }
    if (isDbSelector) {
        ...
        if (searchClause2) {
            params +=
                CommonParams.get("arg_separator") +
                "searchClause2=" +
                encodeURIComponent(searchClause2);
        }
    }
        ...
        if (typeof data !== "undefined" && data.success) {
            ...
            if (isDbSelector) {
                ...
                $this
                    ...
                    .html($(data.message).children().show());
                if (val) {
                    $parent.find("li.fast_filter input.searchClause").val(val);
                }
                $parent
                    .find("span.pos2_value")
                    .first()
                    .text($parent.find("span.pos2_value").last().text());
                $parent
                    .find("span.pos3_value")
                    .first()
                    .text($parent.find("span.pos3_value").last().text());
            }
        } else {
            Functions.ajaxShowMessage(data.error);
...
Navigation.ResizeHandler = function () {
    ...
    this.mouseup = function (event) {
        ...
        Functions.configSet(
            "NavigationWidth",
            event.data.resize_handler.getPos(event),
        );
    ...
    this.treeInit = function () {
        ...
        $("#pma_navigation_collapser").html(
            this.getSymbol($("#pma_navigation").width()),
        );
        ...
        if (isLoadedOnMobile === false) {
            ...
            const initialResizeValue = $("#pma_navigation").data(
                "config-navigation-width",
            );
...
Navigation.FastFilter = {
    ...
    getSearchClause: function () {
        var retval = "";
        var $input = $("#pma_navigation_tree").find(
            "li.fast_filter.db_fast_filter input.searchClause",
        );
    ...
    getSearchClause2: function ($this) {
        ...
        var $filterInput = $([]);
        if (
            $filterContainer.find(
                "li.fast_filter:not(.db_fast_filter) input.searchClause",
            ).length !== 0
        ) {
            $filterInput = $filterContainer.find(
                "li.fast_filter:not(.db_fast_filter) input.searchClause",
            );
        }
        var searchClause2 = "";
...
Navigation.FastFilter.Filter.prototype.request = function () {
    ...
    if (self.$this.find("li.fast_filter").find("img.throbber").length === 0) {
        self.$this.find("li.fast_filter").append(
            $('<div class="throbber"></div>').append(
                $("#pma_navigation_content").find("img.throbber").clone().css({
                    visibility: "visible",
                    display: "block",
                }),
            ),
    ...
    if (
        self.$this
            .find("> ul > li > form.fast_filter")
            .first()
            .find("input[name=searchClause]").length === 0
    ) {
        var $input = $("#pma_navigation_tree").find(
            "li.fast_filter.db_fast_filter input.searchClause",
        );
    ...
    self.xhr = $.ajax({
        url:
            "index.php?route=/navigation&ajax_request=1&server=" +
            CommonParams.get("server"),
        type: "post",
        dataType: "json",
        data: params,
    ...
    $containerELem.find(".hover_show_full").on("mouseenter", function () {
        ...
        if (
            ...
            if ($fullNameLayer.length === 0) {
                $("body").append(
                    '<div id="full_name_layer" class="hide"></div>',
                );
                $("#full_name_layer")
