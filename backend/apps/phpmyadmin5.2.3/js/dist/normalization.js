...
function goToFinish1NF() {
    ...
    $("#mainContent h4").html(
        "<h3>" +
            Functions.sprintf(
                Messages.strFinishMsg,
                Functions.escapeHtml(CommonParams.get("table")),
            ) +
            "</h3>",
    );
    $("#mainContent p").html("");
    ...
    $.post(
        ...
        function (data) {
            ...
            for (var pk in primaryKey) {
                $(
                    "#extra input[value='" +
                        Functions.escapeJsString(primaryKey[pk]) +
                        "']",
...
function goTo2NFStep2(pd, primaryKey) {
    $("#newCols").html("");
    $("#mainContent legend").html(
        Messages.strStep + " 2.2 " + Messages.strConfirmPd,
    );
    ...
    for (var dependson in pd) {
        if (dependson !== primaryKey) {
            ...
            extra +=
                '<p class="d-block m-1">' +
                Functions.escapeHtml(dependson) +
                " -> " +
                Functions.escapeHtml(pd[dependson].toString()) +
                "</p>";
        }
    }
    if (!pdFound) {
    ...
    $(".tblFooters").html(
        '<input type="button" class="btn btn-primary" value="' +
            Messages.strBack +
            '" id="backEditPd"><input type="button" class="btn btn-primary" id="goTo2NFFinish" value="' +
            Messages.strGo +
            '">',
    );
...
function goTo3NFStep2(pd, tablesTds) {
    $("#newCols").html("");
    $("#mainContent legend").html(
        Messages.strStep + " 3.2 " + Messages.strConfirmTd,
    );
    ...
    $(".tblFooters").html(
        '<input type="button" class="btn btn-primary" value="' +
            Messages.strBack +
            '" id="backEditPd"><input type="button" class="btn btn-primary" id="goTo3NFFinish" value="' +
            Messages.strGo +
            '">',
    );
...
function processDependencies(primaryKey, isTransitive) {
    ...
    $("#extra form").each(function () {
        ...
        var formId = $(this).attr("id");
        $("#" + formId + " input[type=checkbox]:not(:checked)").prop(
            "checked",
            false,
        );
        dependsOn = "";
...
AJAX.registerOnload("normalization.js", function () {
    ...
    $("#extra").on("click", "#moveRepeatingGroup", function () {
        ...
        if (repeatingCols !== "") {
            var newColName = $("#extra input[type=checkbox]:checked")
                .first()
                .val();
            repeatingCols = repeatingCols.slice(0, -2);
            var confirmStr = Functions.sprintf(
                Messages.strMoveRepeatingGroup,
                Functions.escapeHtml(repeatingCols),
                Functions.escapeHtml(CommonParams.get("table")),
            );
            confirmStr +=
                '<input type="text" name="repeatGroupTable" placeholder="' +
                Messages.strNewTablePlaceholder +
                '">' +
                "( " +
                Functions.escapeHtml(primaryKey.toString()) +
                ', <input type="text" name="repeatGroupColumn" placeholder="' +
                Messages.strNewColumnPlaceholder +
                '" value="' +
                Functions.escapeHtml(newColName) +
                '">)' +
                "</ol>";
