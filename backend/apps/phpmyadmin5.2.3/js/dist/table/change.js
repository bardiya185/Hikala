...
function nullify(theType, urlField, md5Field, multiEdit) {
    ...
    if (
        typeof rowForm.elements["funcs" + multiEdit + "[" + md5Field + "]"] !==
        "undefined"
    ) {
        rowForm.elements[
            "funcs" + multiEdit + "[" + md5Field + "]"
        ].selectedIndex = -1;
    }
    ...
    if (Number(theType) === 1) {
        rowForm.elements[
            "fields" + multiEdit + "[" + md5Field + "]"
        ][1].selectedIndex = -1;
        // Other "ENUM" field
    } else if (Number(theType) === 2) {
        var elts =
            rowForm.elements["fields" + multiEdit + "[" + md5Field + "]"];
...
function isDate(val, tmstmp) {
    ...
    value = arrayVal.join("-");
    var pos = 2;
    var dtexp = new RegExp(
        /^([0-9]{4})-(((01|03|05|07|08|10|12)-((0[0-9])|([1-2][0-9])|(3[0-1])))|((02|04|06|09|11)-((0[0-9])|([1-2][0-9])|30))|((00)-(00)))$/,
    );
    if (value.length === 8) {
        pos = 0;
    }
...
function isTime(val) {
    ...
    var tmexp = new RegExp(
        /^(-)?(([0-7]?[0-9][0-9])|(8[0-2][0-9])|(83[0-8])):((0[0-9])|([1-5][0-9])):((0[0-9])|([1-5][0-9]))(\.[0-9]{1,6}){0,1}$/,
    );
                ...
                function (value) {
                    ...
                    return (
                        value
                            .replace(/ /g, "")
                            .match(
                                /^(((0x[0-9a-f]+)|([+-]?([0-9]*\.?[0-9]+|[0-9]+\.?[0-9]*)(e[+-]?[0-9]+)?))(,|$))+$/i,
                            ) !== null
                    );
                },
...
function verificationsAfterFieldChange(urlField, multiEdit, theType) {
    ...
    var $thisFunction = $(
        "select[name='funcs[multi_edit][" + multiEdit + "][" + urlField + "]']",
    );
    var functionSelected = false;
    if (
        typeof $thisFunction.val() !== "undefined" &&
    ...
    var newSaltBox =
        "<br><input type=text name=salt[multi_edit][" +
        multiEdit +
        "][" +
        urlField +
        "]" +
    ...
    $(
        "input[name='fields_null[multi_edit][" +
            multiEdit +
            "][" +
            urlField +
            "]']",
...
AJAX.registerOnload("table/change.js", function () {
        ...
        jQuery.validator.addMethod(
            ...
            function (value, element, options) {
                ...
                if (theType === "date") {
                    ...
                    return (
                        isDate(dtValue.substring(0, dv), tmstmp) &&
    ...
    $(document).on("click", "input[name='gis_data[save]']", function () {
        var inputName = $("form#gis_data_editor_form")
    ...
    $('select[name="submit_type"]').on("change", function () {
        ...
        autoIncrementColumn.each(function () {
            ...
            var prevValueField = $table.find(
                'input[name="' +
                    thisElemName.replace("auto_increment", "fields_prev") +
                    '"]',
            );
            var valueField = $table.find(
                'input[name="' +
                    thisElemName.replace("auto_increment", "fields") +
                    '"]',
            );
            var previousValue = $(prevValueField).val();
            if (previousValue !== undefined) {
                if (
                    thisElemSubmitTypeVal === "insert" ||
...
function addNewContinueInsertionFields(event) {
     ...
     */
    var columnCount = $("table.insertRowTable")
        .first()
        .find("tr")
        .has("input[name*='fields_name']").length;
    /**
    ...
    if (currRows < targetRows) {
        var tempIncrementIndex = function () {
            ...
            if ($thisElement.is("select[name*='funcs']")) {
                ...
                var nextSaltInput = $thisElement
                    .parent()
                    .next("td")
                    .next("td")
                    .find("input[name*='salt']");
            ...
            if (
                $thisElement.is(".textfield") ||
                $thisElement.is(".char") ||
                $thisElement.is("textarea")
            ) {
                ...
                if (
                    $thisElement
                        .closest("tr")
                        .find("span.column_type")
                        .html() !== "enum"
                ) {
                    $thisElement.val(
                        $thisElement
                            .closest("tr")
                            .find("span.default_value")
                            .html(),
                    );
                }
                $thisElement
        ...
        var tempReplaceAnchor = function () {
            ...
            var newHref = $anchor
                .attr("href")
                .replace(/rownumber=\d+/, newValue);
            $anchor.attr("href", newHref);
        };
        var restoreValue = function () {
            if (
                $(this).closest("tr").find("span.column_type").html() === "enum"
            ) {
        ...
        while (currRows < targetRows) {
            ...
            $lastRow
                ...
                .find(
                    "input[name*=multi_edit],select[name*=multi_edit],textarea[name*=multi_edit]",
                )
            ...
            $newRow.find(".textfield").each(function () {
                if (
                    ...
                    if (
                        $(this).val() ===
                        $(this).closest("tr").find("span.default_value").html()
                    ) {
                        $(this).prop("checked", true);
                    } else {
                        $(this).prop("checked", false);
            ...
            if (currRows === 1) {
                 ...
                 */
                var $lastCheckbox = $("#insertForm")
                    .children("input:checkbox")
                    .last();

                var lastCheckboxName = $lastCheckbox.attr("name");

                var lastCheckboxIndex = parseInt(
                    lastCheckboxName.match(/\d+/),
                    10,
                );

                var newName = lastCheckboxName.replace(
                    /\d+/,
                    lastCheckboxIndex + 1,
                );
                $('<br><div class="clearfloat"></div>').insertBefore(
                    $("table.insertRowTable").last(),
                );
                $lastCheckbox
                    .clone()
                    .attr({
