...
AJAX.registerOnload("designer/move.js", function () {
    ...
    $(document).on("fullscreenchange", function () {
        if (!document.fullscreenElement) {
            $content
                .removeClass("content_fullscreen")
                .css({ width: "auto", height: "auto" });
            $("#osn_tab").css({ width: "auto", height: "auto" });
            $img.attr("src", $img.data("enter")).attr(
                "title",
                $span.data("enter"),
            );
...
DesignerMove.mouseMove = function (e) {
    ...
    if (curClick !== null) {
        ...
        var curX = parseFloat(
            $curClick.attr("data-" + mainDirection) ||
                $curClick.css(mainDirection),
        );
        var curY = parseFloat(
            $curClick.attr("data-top") || $curClick.css("top"),
        );
...
DesignerMove.canvasPos = function () {
    ...
    if (isIe) {
        document.getElementById("canvas").style.width =
            (osnTabWidth - 3 ? osnTabWidth - 3 : 0) + "px";
        document.getElementById("canvas").style.height =
            (osnTabHeight - 3 ? osnTabHeight - 3 : 0) + "px";
    }
};

DesignerMove.osnTabPos = function () {
    osnTabWidth = parseInt(document.getElementById("osn_tab").style.width, 10);
    osnTabHeight = parseInt(
        document.getElementById("osn_tab").style.height,
        10,
    );
};
...
DesignerMove.resizeOsnTab = function () {
    ...
    for (var key in jTabs) {
        var kX =
            parseInt(document.getElementById(key).style[mainDirection], 10) +
            document.getElementById(key).offsetWidth;
        var kY =
            parseInt(document.getElementById(key).style.top, 10) +
            document.getElementById(key).offsetHeight;
        ...
        for (key in contr[K]) {
            ...
            for (key2 in contr[K][key]) {
                ...
                for (key3 in contr[K][key][key2]) {
                    // field name
                    if (
                        !document.getElementById("check_vis_" + key2).checked ||
                        !document.getElementById(
                            "check_vis_" + contr[K][key][key2][key3][0],
                        ).checked
                    ) {
                        // if hide
                        continue;
                    ...
                    var x1Right =
                        x1Left + document.getElementById(key2).offsetWidth;
                    var x2Left = document.getElementById(
                        contr[K][key][key2][key3][0],
                    ).offsetLeft;
                    var x2Right =
                        x2Left +
                        document.getElementById(contr[K][key][key2][key3][0])
                            .offsetWidth;
                    ...
                    var rowOffsetTop = 0;
                    var tabHideButton = document.getElementById(
                        "id_hide_tbody_" + key2,
                    );

                    if (tabHideButton.innerHTML === "v") {
                        var fromColumn = document.getElementById(
                            key2 + "." + key3,
                        );
                        if (fromColumn) {
                    ...
                    var y1 =
                        document.getElementById(key2).offsetTop +
                        rowOffsetTop +
                        heightField;

                    rowOffsetTop = 0;
                    tabHideButton = document.getElementById(
                        "id_hide_tbody_" + contr[K][key][key2][key3][0],
                    );
                    if (tabHideButton.innerHTML === "v") {
                        var toColumn = document.getElementById(
                            contr[K][key][key2][key3][0] +
                                "." +
...
DesignerMove.addTableToTablesList = function (index, tableDom) {
    ...
    var tableEncoded = $(tableDom)
        .find(".small_tab_pref")
        .attr("table_name_url");
    var tableIsChecked =
        $(tableDom).css("display") === "block" ? "checked" : "";
    var checkboxStatus =
        tableIsChecked === "checked" ? Messages.strHide : Messages.strShow;
    var $newTableLine = $(
        "<tr>" +
            '    <td title="' +
            Messages.strStructure +
            '"' +
            '        width="1px"' +
            '        class="L_butt2_1">' +
            '        <img alt=""' +
    ...
    $($newTableLine)
        .find(".scroll_tab_checkbox")
        .on("click", function () {
            $(this).attr("title", function (i, currentvalue) {
                return currentvalue === Messages.strHide
                    ? Messages.strShow
                    : Messages.strHide;
            });
...
DesignerMove.addOtherDbTables = function () {
    ...
    $.post(
        ...
        function (data) {
            $(data.message)
                .find("table.table_results.data.ajax")
                .find("td.data")
                .each(function () {
    ...
    var $form = $('<form action="" class="ajax"></form>')
        .append($selectDb)
        .append($selectTable);
    var modal = DesignerMove.displayModal(
        $form,
        Messages.strAddTables,
        "#designerGoModal",
    );
    $("#designerModalGoButton").on("click", function () {
        ...
        if ($table.length !== 0) {
            Functions.ajaxShowMessage(
                Functions.sprintf(
                    Messages.strTableAlreadyExists,
                    db + "." + table,
                ),
                undefined,
                "error",
        ...
        $.post(
            ...
            function (data) {
                ...
                var dbEncoded = $($newTableDom)
                    .find(".small_tab_pref")
                    .attr("db_url");
                var tableEncoded = $($newTableDom)
                    .find(".small_tab_pref")
                    .attr("table_name_url");

                if (
                    typeof dbEncoded === "string" &&
                    typeof tableEncoded === "string"
                ) {
...
DesignerMove.save = function (url) {
    for (var key in jTabs) {
        document.getElementById("t_x_" + key + "_").value = parseInt(
            document.getElementById(key).style.left,
            10,
        );
        document.getElementById("t_y_" + key + "_").value = parseInt(
            document.getElementById(key).style.top,
            10,
        );
        document.getElementById("t_v_" + key + "_").value =
            document.getElementById("id_tbody_" + key).style.display === "none"
                ? 0
                : 1;
        document.getElementById("t_h_" + key + "_").value =
            document.getElementById("check_vis_" + key).checked ? 1 : 0;
    }
    ...
    if (designerTablesEnabled || forceString) {
        ...
        for (key in jTabs) {
            poststr +=
                ...
                i +
                "]=" +
                parseInt(document.getElementById(key).style.left, 10);
            poststr +=
                argsep +
                "t_y[" +
                i +
            ...
            poststr +=
                ...
                i +
                "]=" +
                (document.getElementById("id_tbody_" + key).style.display ===
                "none"
                    ? 0
                    : 1);
            poststr +=
...
DesignerMove.save2 = function (callback) {
    if (designerTablesEnabled) {
        ...
        DesignerPage.saveToSelectedPage(
            db,
            selectedPage,
            name,
            DesignerMove.getUrlPos(),
            function () {
                DesignerMove.markSaved();
                if (typeof callback !== "undefined") {
                    callback();
...
DesignerMove.submitSaveDialogAndClose = function (callback, modal) {
    ...
    if (designerTablesEnabled) {
        ...
        $.post(
            $form.attr("action"),
            $form.serialize() + DesignerMove.getUrlPos(),
            function (data) {
                if (data.success === false) {
                    Functions.ajaxShowMessage(data.error, false);
                } else {
...
DesignerMove.save3 = function (callback) {
    if (selectedPage !== -1) {
        ...
        var $form = $(
            '<form action="index.php?route=/database/designer" method="post" name="save_page" id="save_page" class="ajax"></form>',
        )
            .append(
        ...
        $.post(
            ...
            function (data) {
                if (data.success === false) {
                    ...
                    var modal = DesignerMove.displayModal(
                        data.message,
                        Messages.strOpenPage,
                        "#designerGoModal",
                    );
                    $("#designerModalGoButton").on("click", function () {
                        var $form = $("#edit_delete_pages");
                        var selected = $form
                            .find('select[name="selected_page"]')
                            .val();
                        if (selected === "0") {
                            Functions.ajaxShowMessage(
                                Messages.strSelectPage,
                                2000,
                            );
                            return;
                        }
        ...
        function (data) {
            if (data.success === false) {
                ...
                $("#designerModalGoButton").on("click", function () {
                    ...
                    var deletingCurrentPage =
                        parseInt(selected) === selectedPage;
                    Functions.prepareForAjaxRequest($form);
                            ...
                            function (data) {
                                if (data.success === false) {
                                    ...
                                    if (deletingCurrentPage) {
                                        ...
                                        Functions.ajaxShowMessage(
                                            Messages.strSuccessfulPageDelete,
                                        );
                                    }
                                }
                            },
    ...
    $.post(
        ...
        function (data) {
            if (data.success === false) {
                ...
                var modal = DesignerMove.displayModal(
                    data.message,
                    Messages.strSavePageAs,
                    "#designerGoModal",
                );
                $("#designerModalGoButton").on("click", function () {
                    var $form = $("#save_as_pages");
                    var selectedValue = $form
                        .find('input[name="selected_value"]')
                        .val()
                        .trim();
                    var $selectedPage = $form.find(
                        'select[name="selected_page"]',
                    );
                    var choice = $form
                        .find('input[name="save_page"]:checked')
                        .val();
                    var name = "";
                    ...
                    if (choice === "same") {
                        ...
                        if (selectedValue === "") {
                            Functions.ajaxShowMessage(
                                Messages.strEnterValidPageName,
                                2000,
                            );
                            return;
                        }
                    ...
                    if (designerTablesEnabled) {
                        ...
                        if (choice === "same") {
                            var selectedPageId = $selectedPage
...
DesignerMove.promptToSaveCurrentPage = function (callback) {
    if (change === 1 || selectedPage === -1) {
        var modal = DesignerMove.displayModal(
            "<div>" + Messages.strLeavingPage + "</div>",
            Messages.strSavePage,
            "#designerPromptModal",
        );
    ...
    $.post(
        ...
        function (data) {
            if (data.success === false) {
                ...
                if (!designerTablesEnabled) {
                    $form.append(
                        '<input type="hidden" name="offline_export" value="true">',
                    );
                }
                $.each(
                    DesignerMove.getUrlPos(true).substring(1).split(argsep),
                    function () {
                        var pair = this.split("=");
                ...
                var modal = DesignerMove.displayModal(
                    $form,
                    Messages.strExportRelationalSchema,
                    "#designerGoModal",
                );
                $("#designerModalGoButton").on("click", function () {
...
DesignerMove.loadPage = function (page) {
    if (designerTablesEnabled) {
        ...
        $(
            '<a href="index.php?route=/database/designer&server=' +
                server +
                argsep +
                "db=" +
                encodeURIComponent(db) +
...
DesignerMove.angularDirect = function () {
    ...
    if (onAngularDirect) {
        onAngularDirect = 0;
        valueSent = "angular";
        document.getElementById("angular_direct_button").className =
            "M_butt_Selected_down";
    } else {
        onAngularDirect = 1;
        valueSent = "direct";
...
DesignerMove.startRelation = function () {
    ...
    if (!onRelation) {
        ...
        document.getElementById("designer_hint").innerHTML =
            Messages.strSelectReferencedKey;
        document.getElementById("designer_hint").style.display = "block";
        document.getElementById("rel_button").className =
            "M_butt_Selected_down";
    } else {
        document.getElementById("designer_hint").innerHTML = "";
...
DesignerMove.clickField = function (db, T, f, pk) {
    ...
    if (onRelation) {
        if (!clickField) {
            ...
            clickField = 1;
            linkRelation =
                "DB1=" + db + argsep + "T1=" + T + argsep + "F1=" + f;
            document.getElementById("designer_hint").innerHTML =
                Messages.strSelectForeignKey;
        } else {
            ...
            var left =
                globX -
                (document.getElementById("layer_new_relation").offsetWidth >>
                    1);
            document.getElementById("layer_new_relation").style.left =
                left + "px";
            var top =
                globY -
                document.getElementById("layer_new_relation").offsetHeight;
            document.getElementById("layer_new_relation").style.top =
                top + "px";
            document.getElementById("layer_new_relation").style.display =
                "block";
            linkRelation +=
                argsep + "DB2=" + db + argsep + "T2=" + T + argsep + "F2=" + f;
        }
    }
    ...
    if (onDisplayField) {
        ...
        var oldTabField = document.getElementById(
            "id_tr_" + T + "." + displayField[T],
        );
        // if is display field
        if (displayField[T] === f) {
            ...
            var tabField = document.getElementById(
                "id_tr_" + T + "." + displayField[T],
            );
            if (tabField) {
...
DesignerMove.newRelation = function () {
    ...
    var argsep = CommonParams.get("arg_separator");
    linkRelation +=
        argsep + "server=" + server + argsep + "db=" + db + argsep + "db2=p";
    linkRelation +=
        argsep +
        "on_delete=" +
        document.getElementById("on_delete").value +
        argsep +
        "on_update=" +
        document.getElementById("on_update").value;
    linkRelation +=
        argsep + "operation=addNewRelation" + argsep + "ajax_request=true";
...
DesignerMove.smallTabAll = function (idThis) {
    ...
    if (icon.alt === "v") {
        $(".designer_tab .small_tab,.small_tab2").each(
...
DesignerMove.selectTab = function (t) {
    ...
    window.scrollTo(
        parseInt(idT.style.left, 10) - 300,
        parseInt(idT.style.top, 10) - 300,
    );
    setTimeout(function () {
        document.getElementById("id_zag_" + t).className = "tab_zag";
    }, 800);
...
DesignerMove.canvasClick = function (id, event) {
        ...
        for (key in contr[K]) {
            for (key2 in contr[K][key]) {
                for (key3 in contr[K][key][key2]) {
                    ...
                    var x2Left = document.getElementById(
                        contr[K][key][key2][key3][0],
                    ).offsetLeft; // +document.getElementById(contr[K][key2][key3][0]+"."+contr[K][key2][key3][1]).offsetLeft
                    var x2Right =
                        x2Left +
                        document.getElementById(contr[K][key][key2][key3][0])
                            .offsetWidth;
                    ...
                    var y1 =
                        document.getElementById(key2).offsetTop +
                        document.getElementById(key2 + "." + key3).offsetTop +
                        heightField;
                    var y2 =
                        document.getElementById(contr[K][key][key2][key3][0])
                            .offsetTop +
    ...
    if (selected) {
        ...
        var left =
            globX -
            (document.getElementById("layer_upd_relation").offsetWidth >> 1);
        document.getElementById("layer_upd_relation").style.left = left + "px";
        var top =
            globY -
            document.getElementById("layer_upd_relation").offsetHeight -
            10;
...
DesignerMove.updRelation = function () {
    ...
    linkRelation +=
        argsep + "operation=removeRelation" + argsep + "ajax_request=true";
...
DesignerMove.hideTabAll = function (idThis) {
    ...
    for (var i = 0; i < EelementsLength; i++) {
        if (
            ...
            E.elements[i].id.substring(0, 10) === "check_vis_"
        ) {
            if (idThis.alt === "v") {
...
DesignerMove.generalScroll = function () {
    ...
    timeoutId = setTimeout(function () {
        // eslint-disable-next-line compat/compat
        document.getElementById("top_menu").style.left =
            document.body.scrollLeft + "px";
        // eslint-disable-next-line compat/compat
        document.getElementById("top_menu").style.top =
...
DesignerMove.sideMenuRight = function (idThis) {
    ...
    menuMoved = !menuMoved;
    DesignerMove.saveValueInConfig(
        "side_menu",
        $("#side_menu").hasClass("right"),
    );
...
DesignerMove.startDisplayField = function () {
    ...
    if (!onDisplayField) {
        onDisplayField = 1;
        document.getElementById("designer_hint").innerHTML =
            Messages.strChangeDisplay;
        document.getElementById("designer_hint").style.display = "block";
        document.getElementById("display_field_button").className =
...
DesignerMove.clickOption = function (
    dbName,
    tableName,
    columnName,
    tableDbNameUrl,
    ...
    document.getElementById("ok_add_object_db_and_table_name_url").value =
        tableDbNameUrl;
...
DesignerMove.selectAll = function (tableName, dbName, idSelectAll) {
    ...
    var checkboxAll = $(
        "#container-form input[id_check_all='" + idSelectAll + "']:checkbox",
    );
...
DesignerMove.tableOnOver = function (idThis, val, buil) {
    ...
    if (!val) {
        ...
        if (builLocal) {
            document.getElementById("id_zag_" + idThis + "_2").className =
                "tab_zag_2";
        }
    } else {
...
DesignerMove.addObject = function (dbName, tableName, colName, dbTableNameUrl) {
    ...
    if (rel.value !== "--") {
        if (document.getElementById("Query").value === "") {
            Functions.ajaxShowMessage(
                Functions.sprintf(Messages.strQueryEmpty),
            );
            return;
        }
        ...
        historyArray.push(
            new DesignerHistory.HistoryObj(
                colName,
                whereObj,
                tableName,
                hTabs[dbTableNameUrl],
                "Where",
            ),
        );
        sum = sum + 1;
    ...
    if (document.getElementById("new_name").value !== "") {
        var renameObj = new DesignerHistory.Rename(
            document.getElementById("new_name").value,
        ); // make Rename object
        historyArray.push(
            new DesignerHistory.HistoryObj(
    ...
    if (document.getElementById("operator").value !== "---") {
        var aggregateObj = new DesignerHistory.Aggregate(
            document.getElementById("operator").value,
        );
        historyArray.push(
            new DesignerHistory.HistoryObj(
    ...
    if (document.getElementById("orderby").value !== "---") {
        var orderByObj = new DesignerHistory.OrderBy(
            document.getElementById("orderby").value,
        );
        historyArray.push(
            new DesignerHistory.HistoryObj(
    ...
    Functions.ajaxShowMessage(
        Functions.sprintf(Messages.strObjectsCreated, sum),
    );
...
DesignerMove.enableTableEvents = function (index, element) {
    ...
    $(element).on("click", ".small_tab_pref_1", function () {
        DesignerMove.startTabUpd(
            $(this).attr("db_url"),
            $(this).attr("table_name_url"),
        );
    });
    $(element).on("click", ".select_all_store_col", function () {
        DesignerMove.storeColumn(
            $(this).attr("table_name"),
            $(this).attr("col_name"),
            $(this).attr("id"),
        );
    ...
    $(element)
        .find(".tab_zag_noquery")
        .on("mouseover", function () {
            DesignerMove.tableOnOver(
                $(this).attr("table_name"),
                0,
                $(this).attr("query_set"),
            );
...
AJAX.registerTeardown("designer/move.js", function () {
    ...
    $("#id_scroll_tab")
        .find("tr")
        .off("click", ".designer_Tabs2,.designer_Tabs");
