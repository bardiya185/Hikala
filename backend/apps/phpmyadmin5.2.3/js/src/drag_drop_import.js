...
var DragDropImport = {
    ...
    getExtension: function (file) {
        ...
        if (
            jQuery.inArray(
                ext.toLowerCase(),
                DragDropImport.allowedCompressedExtensions,
            ) !== -1
        ) {
        ...
        if (
            jQuery.inArray(
                ext.toLowerCase(),
                DragDropImport.allowedExtensions,
            ) !== -1
        ) {
            return ext;
        }
    ...
    sendFileToServer: function (formData, hash) {
        var jqXHR = $.ajax({
            ...
            success: function (data) {
                ...
                if (!data.success) {
                    DragDropImport.importStatus[
                        DragDropImport.importStatus.length
                    ] = {
                        hash: hash,
                        message: data.error,
                    };
        ...
        $(
            '.pma_sql_import_status div li[data-hash="' +
                hash +
                '"] span.filesize',
        ).html(
            '<span hash="' +
                hash +
                '" class="pma_drop_file_status" task="cancel">' +
                Messages.dropImportMessageCancel +
                "</span>",
        );
        ...
        $(
            ...
            if ($(this).attr("task") === "cancel") {
                jqXHR.abort();
                $(this).html(
                    "<span>" + Messages.dropImportMessageAborted + "</span>",
                );
                ...
                $.each(DragDropImport.importStatus, function (key, value) {
                    if (value.hash === hash) {
                        ...
                        var filename = $this
                            ...
                            .attr("data-filename");
                        $("body").append(
                            '<div class="pma_drop_result"><h2>' +
                                Messages.dropImportImportResultHeader +
                                " - " +
                                Functions.escapeHtml(filename) +
    ...
    hasFiles: function (event) {
        return !(
            typeof event.originalEvent.dataTransfer.types === "undefined" ||
    ...
    importFinished: function (hash, aborted, status) {
        ...
        // Set icon
        $('.pma_sql_import_status div li[data-hash="' + hash + '"]').prepend(
            '<img src="./themes/dot.gif" title="finished" class="' +
                icon +
                '"> ',
        );
    ...
    drop: function (event) {
        ...
        if (dbname !== "") {
            ...
            for (var i = 0; i < files.length; i++) {
                ...
                $sqlImportStatusDiv.append(
                    '<li data-hash="' +
                        hash +
                        '">' +
                        (ext !== ""
                            ? ""
                            : '<img src="./themes/dot.gif" title="invalid format" class="icon ic_s_notice"> ') +
                        Functions.escapeHtml(files[i].name) +
                        '<span class="filesize" data-filename="' +
                        Functions.escapeHtml(files[i].name) +
                        '">' +
                ...
                if (ext !== "") {
                    ...
                    $(".pma_import_count").html(
                        ++DragDropImport.liveUploadCount,
                    );
