...
$.ajaxPrefilter(function (options, originalOptions) {
    ...
    if (typeof options.data === "string") {
        options.data +=
            ...
            nocache +
            "&token=" +
            encodeURIComponent(CommonParams.get("token"));
...
Functions.addDatepicker = function ($thisElement, type, options) {
    ...
    if (type === "time") {
        ...
        Functions.tooltip(
            $thisElement,
            "input",
            Messages.strMysqlAllowedValuesTipTime,
        );
    } else {
...
Functions.addDateTimePicker = function () {
    if ($.timepicker !== undefined) {
        $("input.timefield, input.datefield, input.datetimefield").each(
            function () {
                ...
                if ($(this).hasClass("timefield")) {
                    ...
                    Functions.tooltip(
                        $(this),
                        "input",
                        Messages.strMysqlAllowedValuesTipDate,
                    );
                }
            },
        );
    ...
    if (parseInt(data.redirect_flag) === 1) {
        ...
        if (window.location.href.indexOf("?") === -1) {
            ...
            window.location.href +=
                CommonParams.get("arg_separator") + "session_expired=1";
        }
...
Functions.getSqlEditor = function ($textarea, options, resize, lintOptions) {
    ...
    if ($textarea.length > 0 && typeof CodeMirror !== "undefined") {
        ...
        codemirrorEditor.on(
            "inputRead",
            Functions.codeMirrorAutoCompleteOnInputRead,
        );
...
Functions.prepareForAjaxRequest = function ($form) {
    if (!$form.find("input:hidden").is("#ajax_request_hidden")) {
        $form.append(
            '<input type="hidden" id="ajax_request_hidden" name="ajax_request" value="true">',
        );
    }
};

Functions.checkPasswordStrength = function (
    value,
    meterObject,
    meterObjectLabel,
    username,
    ...
    zxcvbnts.core.zxcvbnOptions.setOptions({
        dictionary: { userInputs: customDict },
    });
...
Functions.suggestPassword = function (passwordForm) {
    ...
    var pwchars =
        "abcdefghijklmnopqrstuvwxyz0123456789ABCDEFGHIJKLMNOPQRSTUVWYXZ@!_.*/()[]-";
    ...
    for (i = 0; i < passwordlength; i++) {
        passwd.value += pwchars.charAt(
            Math.abs(randomWords[i]) % pwchars.length,
        );
    }
    ...
    var meterObjLabel = $jQueryPasswordForm
        .find('span[name="pw_strength"]')
        .first();
    var username = "";
    if (passwordForm.elements.username) {
        username = passwordForm.elements.username.value;
    }
    Functions.checkPasswordStrength(
        passwd.value,
        meterObj,
        meterObjLabel,
        username,
...
Functions.displayPasswordGenerateButton = function () {
    ...
    if (document.getElementById("button_generate_password") === null) {
        $("#div_element_before_generate_password")
            .parent()
            .append(generatePwdDiv);
    }
};
...
Functions.confirmLink = function (theLink, theSqlQuery) {
    ...
    var isConfirmed = confirm(
        Functions.sprintf(Messages.strDoYouReally, theSqlQuery),
    );
    if (isConfirmed) {
        if (typeof theLink.href !== "undefined") {
            theLink.href +=
                CommonParams.get("arg_separator") + "is_js_confirmed=1";
...
Functions.confirmQuery = function (theForm1, sqlQuery1) {
    ...
    var doConfirmRegExp0 = new RegExp(
        "^\\s*DROP\\s+(IF EXISTS\\s+)?(TABLE|PROCEDURE)\\s",
        "i",
    );
    var doConfirmRegExp1 = new RegExp(
        "^\\s*ALTER\\s+TABLE\\s+((`[^`]+`)|([A-Za-z0-9_$]+))\\s+DROP\\s",
        "i",
    );
    ...
    if (
        doConfirmRegExp0.test(sqlQuery1) ||
        ...
        var isConfirmed = confirm(
            Functions.sprintf(Messages.strDoYouReally, message),
        );
...
Functions.checkSqlQuery = function (theForm) {
    ...
    if (
        typeof theForm.elements.sql_file !== "undefined" &&
        theForm.elements.sql_file.value.replace(spaceRegExp, "") !== ""
    ) {
        return true;
    }
    if (
        typeof theForm.elements.id_bookmark !== "undefined" &&
        (theForm.elements.id_bookmark.value !== null ||
            theForm.elements.id_bookmark.value !== "") &&
 ...
 */
Functions.checkFormElementInRange = function (
    theForm,
    theFieldName,
    message,
    minimum,
                ...
                if (data.success) {
                    ...
                    if (!$("#modalOverlay").length) {
                        $("fieldset")
                            ...
                            .addClass("disabled_for_expiration");
                        $("body").append(data.error);
                        $(".ui-dialog").each(function () {
                            $("#" + $(this).attr("aria-describedby")).dialog(
                                "close",
                            );
                        });
...
AJAX.registerOnload("functions.js", function () {
    ...
    $(document).on("click", "input:checkbox.checkall", function (e) {
        ...
        if (!e.shiftKey || lastClickedRow === -1) {
            ...
            lastClickedRow = lastClickChecked
                ? $table.find("tbody tr:not(.noclick)").index($tr)
                : -1;
...
Functions.setSelectOptions = function (theForm, theSelect, doCheck) {
    $("form[name='" + theForm + "'] select[name='" + theSelect + "']")
        .find("option")
        .prop("selected", doCheck);
    return true;
};
...
Functions.handleSimulateQueryButton = function () {
    var updateRegExp =
        /^\s*UPDATE\b\s*(((`([^`]|``)+`)|([a-z0-9_$]+))\s*\.\s*)?((`([^`]|``)+`)|([a-z0-9_$]+))\s*\bSET\b/i;
    var deleteRegExp =
        /^\s*DELETE\b\s*((((`([^`]|``)+`)|([a-z0-9_$]+))\s*\.\s*)?((`([^`]|``)+`)|([a-z0-9_$]+))\s*)?\bFROM\b/i;

    var query = codeMirrorEditor
...
Functions.insertQuery = function (queryType) {
    ...
    if (myListBox.options.length > 0) {
        ...
        for (var i = 0; i < myListBox.options.length; i++) {
            ...
            editDis +=
                myListBox.options[i].value + "='[value-" + NbSelect + "]'";
        }
...
Functions.insertValueQuery = function () {
    ...
    if (myListBox.options.length > 0) {
        ...
        if (codeMirrorEditor) {
            ...
            document.sqlform.sql_query.selectionStart ||
            document.sqlform.sql_query.selectionStart === "0"
        ) {
            ...
            myQuery.value =
                SqlString.substring(0, startPos) +
                columnsList +
                SqlString.substring(endPos, SqlString.length);
            myQuery.focus();
        } else {
...
Functions.updateQueryParameters = function () {
    if ($("#parameterized").is(":checked")) {
        ...
        $.each(parameters, function (i, parameter) {
            ...
            if (!$param.length) {
                $param = $(
                    '<span class="parameter" id="paramSpan_' +
                        paramName +
                        '"></span>',
                );
                $('<label for="param_' + paramName + '"></label>')
                    .text(parameter)
                    .appendTo($param);
                $(
...
Functions.loadForeignKeyCheckbox = function () {
    ...
    $.get(
        "index.php?route=/sql/get-default-fk-check-value",
        params,
        function (data) {
            var html =
...
Functions.getJsConfirmCommonParam = function (elem, parameters) {
    ...
    params +=
        ...
        sep +
        "fk_checks=" +
        ($elem.find("#fk_checks").is(":checked") ? 1 : 0);
    return params;
};
...
AJAX.registerOnload("functions.js", function () {
    ...
    $(document).on("click", "a.inline_edit_sql", function () {
        ...
        var newContent =
            '<textarea name="sql_query_edit" id="sql_query_edit">' +
            Functions.escapeHtml(sqlQuery) +
            "</textarea>\n";
        newContent += Functions.getForeignKeyCheckboxLoader();
        newContent +=
            '<input type="submit" id="sql_query_edit_save" class="btn btn-secondary button btnSave" value="' +
            Messages.strGo +
            '">\n';
        newContent +=
            '<input type="button" id="sql_query_edit_discard" class="btn btn-secondary button btnDiscard" value="' +
            Messages.strCancel +
            '">\n';
    ...
    $(document).on("click", "input#sql_query_edit_save", function () {
        ...
        var $fakeForm = $("<form>", {
            ...
            .append(
                $form
                    .find(
                        "input[name=server], input[name=db], input[name=table], input[name=token]",
                    )
                    .clone(),
            )
            .append(
...
Functions.bindCodeMirrorToInlineEditor = function () {
    ...
    if ($inlineEditor.length > 0) {
        if (typeof CodeMirror !== "undefined") {
            ...
            codeMirrorInlineEditor.focus();
            $(codeMirrorInlineEditor.getWrapperElement()).on(
                "keydown",
                Functions.catchKeypressesFromSqlInlineEdit,
            );
        } else {
            $inlineEditor
                .trigger("focus")
...
Functions.documentationAdd = function ($elm, params) {
    ...
    $elm.text("");
    $elm.append(
        '<a target="mysql_doc" class="cm-sql-doc" href="' +
            url +
            '">' +
            content +
            "</a>",
    ...
    $elm.each(function () {
        ...
        if ($pre.is(":visible")) {
            ...
            if (typeof CodeMirror !== "undefined") {
                ...
                $highlight
                    .find(".cm-keyword")
                    .each(Functions.documentationKeyword);
                $highlight
                    .find(".cm-builtin")
                    .each(Functions.documentationBuiltin);
            }
        }
    });
};
...
Functions.updateCode = function ($base, htmlValue, rawValue) {
    ...
    if (typeof CodeMirror !== "undefined") {
        var $highlighted = $(
            '<div class="' + type + '-highlight cm-s-default"></div>',
        );
...
$(function () {
    ...
    $(document).on(
        "mousedown",
        "span.ajax_notification.dismissable",
        function () {
    ...
    $(document).on(
        ...
        function () {
            if (
                $(this).parents("span.ajax_notification").is(":data(tooltip)")
            ) {
    ...
    $(document).on("click", "a.copyQueryBtn", function (event) {
        ...
        if (res) {
            $(this).after(
                "<span id='copyStatus'> (" +
                    Messages.strCopyQueryButtonSuccess +
                    ")</span>",
            );
        } else {
            $(this).after(
                "<span id='copyStatus'> (" +
                    Messages.strCopyQueryButtonFailure +
                    ")</span>",
            );
        }
        setTimeout(function () {
    ...
    $(document).on(
        ...
        function (event) {
            ...
            if (event.type === "mouseover") {
                message = $(this).hasClass("copyQueryBtn")
...
Functions.showNoticeForEnum = function (selectElement) {
    ...
    enumNoticeId +=
        "_" + (parseInt(selectElement.attr("id").split("_")[2], 10) + 1);
...
Functions.showWarningForIntTypes = function () {
    if ($("div#length_not_allowed").length) {
        var lengthRestrictions = $("select.column_type option")
            .map(function () {
                return $(this)
                    .filter(":selected")
                    .attr("data-length-restricted");
            })
            .get();

        var restricationFound = lengthRestrictions.some(
            (restriction) => Number(restriction) === 1,
        );
...
Functions.sqlPrettyPrint = function (string) {
    ...
    for (var i = 0; i < tokens.length; i++) {
        ...
        if (statementPart !== -1) {
            ...
            if (
                !spaceExceptionsBefore[tokens[i][1]] &&
        ...
        if (
            (lastStatementPart === "select" ||
                lastStatementPart === "where" ||
                lastStatementPart === "set") &&
            tokens[i][1] === "," &&
            blockStack[0] === "statement"
        ) {
            output += "\n" + tabs(indentLevel + 1);
        }
...
AJAX.registerTeardown("functions.js", function () {
    ...
    $(document).off(
        "click",
        "form.create_table_form.ajax input[name=submit_num_fields]",
    );
    $(document).off("keyup", "form.create_table_form.ajax input");
    $(document).off(
        "change",
        "input[name=partition_count],input[name=subpartition_count],select[name=partition_by]",
    );
});
...
AJAX.registerOnload("functions.js", function () {
    ...
    $(document).on("submit", "form.create_table_form.ajax", function (event) {
        ...
        if (
            Functions.checkTableEditForm(
                $form[0],
                $form.find("input[name=orig_num_fields]").val(),
            )
        ) {
                    ...
                    function (data) {
                        if (
                            ...
                            if (tablesTable.length === 0) {
                                 ...
                                 */
                                var currLastRowIndex = parseFloat(
                                    currLastRowIndexString,
                                );
                                /**
                                 ...
                                 */
                                var newLastRowId =
                                    "checkbox_tbl_" + newLastRowIndex;

                                data.newTableString =
                                    data.newTableString.replace(
                                        /checkbox_tbl_/,
                                        newLastRowId,
                                    );
                                // append to table
        ...
        $.post(
            ...
            function (data) {
                if (typeof data !== "undefined" && data.success) {
                    ...
                    Functions.hideShowConnection(
                        $(".create_table_form select[name=tbl_storage_engine]"),
                    );
    ...
    $(document).on(
        "keydown",
        "form.create_table_form.ajax input[name=added_fields]",
        function (event) {
            if (event.keyCode === 13) {
                event.preventDefault();
...
Functions.autoPopulate = function (inputId, offset) {
    ...
    $("#" + newInputId + "1").val(
        centralColumnList[db + "_" + table][offset].col_name,
    );
    var colType =
        centralColumnList[db + "_" + table][offset].col_type.toUpperCase();
    ...
    var colDefault =
        centralColumnList[db + "_" + table][offset].col_default.toUpperCase();
    ...
    if (colDefault === "") {
        ...
        $input4
            .siblings(".default_value")
            .val(centralColumnList[db + "_" + table][offset].col_default);
    }
    $("#" + newInputId + "5").val(
        centralColumnList[db + "_" + table][offset].col_collation,
    );
    var $input6 = $("#" + newInputId + "6");
    $input6.val(centralColumnList[db + "_" + table][offset].col_attribute);
    if (
        centralColumnList[db + "_" + table][offset].col_extra ===
        "on update CURRENT_TIMESTAMP"
    ) {
        $input6.val(centralColumnList[db + "_" + table][offset].col_extra);
    }
    if (
        centralColumnList[db + "_" + table][offset].col_extra.toUpperCase() ===
        "AUTO_INCREMENT"
    ) {
        $("#" + newInputId + "9")
            .prop("checked", true)
...
AJAX.registerOnload("functions.js", function () {
    $(document).on("click", "a.open_enum_editor", function () {
        ...
        var dialog =
            ...
            "</legend>" +
            "<p>" +
            Functions.getImage("s_notice") +
            Messages.enum_hint +
            "</p>" +
            '<table class="table table-borderless values">' +
            fields +
            "</table>" +
    ...
    $(document).on("click", "a.central_columns_dialog", function () {
        ...
        for (i = 0; i < min; i++) {
            ...
            if (centralColumnList[db + "_" + table][i].col_attribute !== "") {
                fields +=
                    "(" +
                    Functions.escapeHtml(
                        centralColumnList[db + "_" + table][i].col_attribute,
                    ) +
                    ") ";
            }
            if (centralColumnList[db + "_" + table][i].col_length !== "") {
                fields +=
                    "(" +
                    Functions.escapeHtml(
                        centralColumnList[db + "_" + table][i].col_length,
                    ) +
                    ") ";
            }
            fields +=
                Functions.escapeHtml(
                    centralColumnList[db + "_" + table][i].col_extra,
                ) +
                "</span>" +
                "</div></td>";
            if (pick) {
                fields +=
                    '<td><input class="btn btn-secondary pick w-100" type="submit" value="' +
                    Messages.pickColumn +
                    '" onclick="Functions.autoPopulate(\'' +
                    colid +
                    "'," +
                    i +
                    ')"></td>';
        ...
        var searchIn =
            ...
            '">';
        if (fields === "") {
            fields = Functions.sprintf(
                Messages.strEmptyCentralList,
                "'" + Functions.escapeHtml(db) + "'",
            );
            searchIn = "";
        }
        var seeMore = "";
        if (listSize > maxRows) {
            seeMore =
                '<fieldset class="pma-fieldset tblFooters text-center fw-bold">' +
                "<a href='#' id='seeMore'>" +
                Messages.seeMore +
                "</a></fieldset>";
        }
        var centralColumnsDialog =
            "<div class='max_height_400'>" +
            '<fieldset class="pma-fieldset">' +
            searchIn +
            '<table id="col_list" class="table table-borderless values">' +
            fields +
            "</table>" +
            "</fieldset>" +
            seeMore +
        ...
        var $centralColumnsDialog = $(centralColumnsDialog).dialog({
            ...
            open: function () {
                ...
                $("#seeMore").on("click", function () {
                    fields = "";
                    min =
                        listSize <= maxRows + resultPointer
                            ? listSize
                            : maxRows + resultPointer;
    ...
    $(document).on("click", "input.add_value", function (e) {
        e.preventDefault();
        var numNewRows = $("#enumEditorModal")
            .find("div.slider")
            .slider("value");
        while (numNewRows--) {
            $("#enumEditorModal")
        ...
        if (hadAddButtonHidden === false) {
            ...
            while (rowsToAdd--) {
                ...
                var $newrow = $indexColumns
                    ...
                    .first()
                    .clone()
                    .appendTo($indexColumns.find("tbody"));
...
Functions.indexDialogModal = function (
    routeUrl,
    url,
    title,
    callbackSuccess,
    ...
    const indexDialogPreviewModal = document.getElementById(
        "indexDialogPreviewModal",
    );
    ...
    indexDialogPreviewModal.addEventListener("hidden.bs.modal", () => {
        indexDialogPreviewModal.querySelector(".modal-body").innerHTML =
        ...
        $.post(
            ...
            function (data) {
                ...
                if (typeof data !== "undefined" && data.success === true) {
                    ...
                    var $tempDiv = $("<div id='temp_div'><div>").append(
                        data.error,
                    );
                    var $error;
...
Functions.indexEditorDialog = function (
    url,
    title,
    callbackSuccess,
    callbackFailure,
) {
    Functions.indexDialogModal(
...
Functions.indexRenameDialog = function (
    url,
    title,
    callbackSuccess,
    callbackFailure,
    ...
    Functions.indexDialogModal(
        "index.php?route=/table/indexes/rename",
        url,
        title,
        callbackSuccess,
        callbackFailure,
    ...
    $indexColumns.find("tbody").sortable({
        ...
        helper: function (event, tr) {
            ...
            $helper.children().each(function (index) {
                ...
                var $childrenSelect = $originalCells
                    .eq(index)
                    .children("select");
    ...
    $outer.find(".slider").slider({
        ...
        slide: function (event, ui) {
            $(this)
                .closest("fieldset")
                .find("input[type=submit]")
                .val(Functions.sprintf(Messages.strAddToIndex, ui.value));
        },
    });
...
AJAX.registerOnload("functions.js", function () {
    ...
    $(
        "#page_content form.lock-page textarea, " +
            ...
            "#page_content form.lock-page select",
    ).each(function (i) {
        $(this).data("lock-id", i);
    ...
    $(
        '#page_content form.lock-page input[type="checkbox"], ' +
            '#page_content form.lock-page input[type="radio"]',
    ).each(function (i) {
        $(this).data("lock-id", i);
...
AJAX.registerTeardown("functions.js", function () {
    ...
    $(document).off(
        "keydown",
        "#createViewModal input, #createViewModal select",
    );
...
AJAX.registerOnload("functions.js", function () {
    ...
    $(".logout").on("click", function () {
        var form = $(
            '<form method="POST" action="' +
                ...
                Functions.escapeHtml(CommonParams.get("token")) +
                '">' +
                "</form>",
        );
        $("body").append(form);
    ...
    if ($('textarea[name="view[as]"]').length !== 0) {
        codeMirrorEditor = Functions.getSqlEditor(
            $('textarea[name="view[as]"]'),
        );
    }
});
    ...
    $.post($this.attr("href"), params, function (data) {
        if (typeof data !== "undefined" && data.success === true) {
            ...
            $("#createViewModalGoButton").on("click", function () {
                ...
                $.post(
                    "index.php?route=/view/create",
                    $("#createViewModal").find("form").serialize(),
                    function (data) {
                        Functions.ajaxRemoveMessage($msg);
                        if (
                            typeof data !== "undefined" &&
            ...
            $("#createViewModal").on("shown.bs.modal", function () {
                codeMirrorEditor = Functions.getSqlEditor(
                    $("#createViewModal").find("textarea"),
                );
                $("input:visible[type=text]", $("#createViewModal"))
                    .first()
                    .trigger("focus");
...
$(function () {
    if (
        $("#floating_menubar").length &&
        $("#PMA_disable_floating_menubar").length === 0
    ) {
...
$(function () {
    $(document).on(
        "click",
        "#server-breadcrumb, #goto_pagetop",
        function (event) {
...
$(document).on("change", "input.checkall_box", function () {
    ...
    $(this.form)
        ...
        .parents("tr")
        .toggleClass("marked table-active", isChecked);
});
...
$(document).on(
    "change",
    checkboxesSel + ", input.checkall_box:checkbox:enabled",
    Functions.subCheckboxesChanged,
);
...
Functions.checkNumberOfFields = function () {
    ...
    $("form").each(function () {
        ...
        if (nbInputs > maxInputVars) {
            var warning = Functions.sprintf(
                Messages.strTooManyInputs,
                maxInputVars,
            );
...
Functions.toggleDatepickerIfInvalid = function ($td, $inputField) {
    ...
    if ($inputField.hasClass("hasDatepicker")) {
        ...
        if (
            $td.attr("data-type") === "date" &&
            !dtexpDate.test($inputField.val())
        ) {
            $inputField.datepicker("hide");
        } else if (
            $td.attr("data-type") === "time" &&
            !dtexpTime.test($inputField.val())
        ) {
            $inputField.datepicker("hide");
        } else {
...
AJAX.registerOnload("functions.js", function () {
    ...
    $("form input, form textarea, form select").on("keydown", function (e) {
        if ((e.ctrlKey && e.which === 13) || (e.altKey && e.which === 13)) {
            ...
            if (
                !$form.find('input[type="submit"]').first() ||
