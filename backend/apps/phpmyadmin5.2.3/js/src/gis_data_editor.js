...
function prepareJSVersion() {
    ...
    $("#gis_data_editor").prepend(
        '<a class="close_gis_editor" href="#">' + Messages.strClose + "</a>",
    );
    $(
...
function loadGISEditor(value, field, type, inputName) {
    ...
    $.post(
        "index.php?route=/gis-data-editor",
        {
            field: field,
            value: value,
            type: type,
...
AJAX.registerOnload("gis_data_editor.js", function () {
    ...
    $(document).on("click", "#gis_editor a.addJs.addPoint", function () {
        ...
        var $noOfPointsInput = $(
            "input[name='" + prefix + "[no_of_points]" + "']",
        );
    ...
    $(document).on("click", "#gis_editor a.addJs.addLine", function () {
        ...
        html +=
            '<a class="btn btn-secondary addPoint addJs" name="' +
            prefix +
            "[" +
            noOfLines +
            '][add_point]" href="#">+ ' +
            Messages.strAddPoint +
            "</a><br>";
    ...
    $(document).on("click", "#gis_editor a.addJs.addPolygon", function () {
        ...
        var $noOfPolygonsInput = $(
            "input[name='" + prefix + "[no_of_polygons]" + "']",
        );
        ...
        html +=
            ...
            Messages.strAddPoint +
            "</a><br>" +
            '<a class="btn btn-secondary addLine addJs" name="' +
            prefix +
            "[" +
            noOfPolygons +
            '][add_line]" href="#">+ ' +
    ...
    $(document).on("click", "#gis_editor a.addJs.addGeom", function () {
        ...
        var $noOfGeomsInput = $(
            "input[name='" + prefix + "[geom_count]" + "']",
        );
        var noOfGeoms = parseInt($noOfGeomsInput.val(), 10);

        var html1 = Messages.strGeometry + " " + (noOfGeoms + 1) + ":<br>";
        var $geomType = $("#gis_type_template")
            .contents()
            .filter("select")
            .clone();
        $geomType
            .attr("name", "gis_data[" + noOfGeoms + "][gis_type]")
            .val("POINT");
        var html2 =
            "<br>" +
            Messages.strPoint +
