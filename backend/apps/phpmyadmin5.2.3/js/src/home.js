const GitInfo = {
    ...
    parseVersionString: function (str) {
        ...
        return (
            maj * 100000000 + min * 1000000 + pat * 10000 + hotfix * 100 + add
        );
    },
    ...
    currentVersion: function (data) {
        if (data && data.version && data.date) {
            const current = GitInfo.parseVersionString(
                $("span.version").text(),
            );
            const latest = GitInfo.parseVersionString(data.version);
            const url =
                "./url.php?url=https://www.phpmyadmin.net/files/" +
                Functions.escapeHtml(encodeURIComponent(data.version)) +
                "/";
            ...
            const versionInformationMessageLinkText = document.createTextNode(
                data.version,
            );
            versionInformationMessageLink.appendChild(
                versionInformationMessageLinkText,
            );
            const prefixMessage = document.createTextNode(
                Messages.strLatestAvailable + " ",
            );
            versionInformationMessage.appendChild(prefixMessage);
            versionInformationMessage.appendChild(
                versionInformationMessageLink,
            );
            if (latest > current) {
                ...
                if (
                    Math.floor(latest / 10000) === Math.floor(current / 10000)
                ) {
                ...
                const mainContainerDivLinkText =
            ...
            if (latest === current) {
                versionInformationMessage = document.createTextNode(
                    " (" + Messages.strUpToDate + ")",
                );
            }
        ...
        $.get("index.php?route=/git-revision", {
            ...
            .fail(function () {
                const gitHashInfoLi =
                    ...
                    window.Messages.errorLoadingGitInformation +
                    "</li>";
