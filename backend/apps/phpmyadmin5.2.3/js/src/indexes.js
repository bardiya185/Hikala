...
Indexes.setIndexFormParameters = function (sourceArray, indexChoice) {
    if (indexChoice === "index") {
        ...
        $('input[name="' + indexChoice + '_indexes"]').val(
            JSON.stringify(sourceArray),
        );
    }
};
...
Indexes.removeColumnFromIndex = function (colIndex) {
    ...
    var previousIndex = $('select[name="field_key[' + colIndex + ']"]').attr(
        "data-index",
    );
    if (previousIndex.length) {
        ...
        if (previousIndex[1] in sourceArray) {
            ...
            for (var i = 0; i < sourceLength; i++) {
                if (i in sourceArray[previousIndex[1]].columns) {
                    if (
                        sourceArray[previousIndex[1]].columns[i].col_index ===
                        colIndex
                    ) {
        ...
        Indexes.setIndexFormParameters(
            sourceArray,
            previousIndex[0].toLowerCase(),
        );
    }
};
 ...
 * @return {void}
 */
Indexes.addColumnToIndex = function (
    sourceArray,
    arrayIndex,
    indexChoice,
    colIndex,
    ...
    $("#index_columns")
        ...
        .each(function () {
            ...
            var colIndex = $(this)
                .find('select[name="index[columns][names][]"]')
                .val();
            var size = $(this)
                .find('input[name="index[columns][sub_parts][]"]')
                .val();
            columns.push({
                col_index: colIndex,
                size: size,
    ...
    if (displayName === "") {
        ...
        $.each(columns, function () {
            columnNames.push(
                $('input[name="field_name[' + this.col_index + ']"]').val(),
            );
        });
    ...
    $.each(columns, function () {
        ...
        if ($name.length === 0) {
            $name = $(
                '<a id="' +
                    id +
                    '" href="#" class="ajax show_index_dialog"></a>',
            );
            $name.insertAfter(
                $('select[name="field_key[' + this.col_index + ']"]'),
            );
        }
    ...
    if (colIndex >= 0) {
        ...
        $('select[name="field_key[' + colIndex + ']"]').attr(
            "data-index",
            indexChoice + "," + arrayIndex,
        );
    }
            ...
            columnNames.push(
                $(
                    'input[name="field_name[' +
                        sourceArray[i].columns[j].col_index +
                        ']"]',
                ).val(),
            );
 ...
 * @return {void}
 */
Indexes.showAddIndexDialog = function (
    sourceArray,
    arrayIndex,
    targetColumns,
    colIndex,
    ...
    for (var i = 0; i < targetColumns.length; i++) {
        var columnName = $(
            'input[name="field_name[' + targetColumns[i] + ']"]',
        ).val();
        var columnType = $(
            'select[name="field_type[' + targetColumns[i] + ']"]',
        )
            .val()
            .toLowerCase();
 ...
 * @return {void}
 */
Indexes.indexTypeSelectionDialog = function (
    sourceArray,
    indexChoice,
    colIndex,
) {
    var $singleColumnRadio = $(
        '<input type="radio" id="single_column" name="index_choice"' +
            ' checked="checked">' +
            '<label for="single_column">' +
            Messages.strCreateSingleColumnIndex +
            "</label>",
    );
    var $compositeIndexRadio = $(
        '<input type="radio" id="composite_index"' +
            ' name="index_choice">' +
            '<label for="composite_index">' +
            Messages.strCreateCompositeIndex +
            "</label>",
    );
    var $dialogContent = $(
        '<fieldset class="pma-fieldset" id="advance_index_creator"></fieldset>',
    );
    ...
    buttonOptions[Messages.strGo].click = function () {
        ...
        if ($("#composite_index").is(":checked")) {
            if (
                $('input[name="composite_with"]').length !== 0 &&
                $('input[name="composite_with"]:checked').length === 0
            ) {
                Functions.ajaxShowMessage(
            ...
            for (var i = 0; i < sourceLength; i++) {
                targetColumns.push(
                    sourceArray[arrayIndex].columns[i].col_index,
                );
            }
    ...
    buttonOptions[Messages.strCancel].click = function () {
        ...
        if (!$selectList.attr("data-index").length) {
            ...
            var previousIndex = $selectList.attr("data-index").split(",");
            $selectList
                .find('option[value*="' + previousIndex[0].toLowerCase() + '"]')
                .attr("selected", "selected");
        }
        $(this).remove();
    };
...
AJAX.registerTeardown("indexes.js", function () {
    ...
    $(document).off(
        "click",
        "#table_index tbody tr td.edit_index.ajax, #index_div .add_index.ajax",
    );
...
AJAX.registerOnload("indexes.js", function () {
    ...
    $(document).on(
        "click",
        "a.drop_primary_key_index_anchor.ajax",
        function (event) {
            event.preventDefault();
            var $anchor = $(this);
            /**
            ...
            for (
                var i = 1, $lastRow = $currRow.next();
                i < rows;
                i++, $lastRow = $lastRow.next()
            ) {
                $rowsToHide = $rowsToHide.add($lastRow);
            }

            var question = $currRow
                .children("td")
                .children(".drop_primary_key_index_msg")
                .val();

            Functions.confirmPreviewSql(
                question,
                $anchor.attr("href"),
                function (url) {
                    var $msg = Functions.ajaxShowMessage(
                        Messages.strDroppingPrimaryKeyIndex,
                        false,
                    );
    ...
    $(document).on(
        "click",
        "#table_index tbody tr td.rename_index.ajax",
        function (event) {
            event.preventDefault();
    ...
    $("body").on(
        ...
        function (e, showDialog) {
            var showDialogLocal =
                typeof showDialog !== "undefined" ? showDialog : true;
            ...
            if (sourceArray.length === 0) {
                ...
                if (indexChoice === "primary") {
                    ...
                    Indexes.indexTypeSelectionDialog(
                        sourceArray,
                        indexChoice,
                        colIndex,
                    );
