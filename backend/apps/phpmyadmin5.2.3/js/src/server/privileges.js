...
function checkAddUser(theForm) {
    ...
    if (
        theForm.elements.pred_username &&
        theForm.elements.pred_username.value === "userdefined" &&
        theForm.elements.username.value === ""
    ) {
    ...
    handleEvent: function (event) {
        ...
        $.get(
            ...
            (data) => {
                ...
                const modalBody =
                    editUserGroupModal.querySelector(".modal-body");
                const saveButton = editUserGroupModal.querySelector(
                    "#editUserGroupModalSaveButton",
                );
                ...
                saveButton.addEventListener("click", () => {
                    const form = $(
                        editUserGroupModal.querySelector(
                            "#changeUserGroupForm",
                        ),
                    );
                    ...
                    $.post(
                        ...
                        (data) => {
                            ...
                            const userGroup = form
                                .serializeArray()
                                .find((el) => el.name === "userGroup").value;
                            const userGroupTableCell =
...
const AccountLocking = {
    handleEvent: function () {
        ...
        $.post(url, params, (data) => {
            ...
            if (isLocked) {
                const lockIcon = Functions.getImage(
                    "s_lock",
                    Messages.strLock,
                    {},
                ).toString();
                ...
                const unlockIcon = Functions.getImage(
                    "s_unlock",
                    Messages.strUnlock,
                    {},
                ).toString();
...
AJAX.registerOnload("server/privileges.js", function () {
                        ...
                        if (
                            ...
                            $form
                                ...
                                .slideUp("medium", function () {
                                    ...
                                    $(this).remove();
                                    if (
                                        $("#userRightsTable").find(
                                            'input:checkbox[value^="' +
                                                thisUserInitial +
                                                '"], input:checkbox[value^="' +
                                                thisUserInitial.toLowerCase() +
                                                '"]',
                                        ).length === 0
                                    ) {
                                    ...
                                    $(Functions.checkboxesSel).trigger(
                                        "change",
                                    );
                                });
                        } else {
    ...
    $(document).on(
        "click",
        "button.mult_submit[value=export]",
        function (event) {
            event.preventDefault();
            if ($(this.form).find("input:checked").length === 0) {
                Functions.ajaxShowMessage(
                    Messages.strNoAccountSelected,
                    2000,
                    "success",
                );
                return;
            ...
            var selectedUsers = $(
                "#usersForm input[name*='selected_usr']:checkbox",
            ).serialize();
            var postStr =
                selectedUsers +
                "&submit_mult=export" +
                argsep +
                "ajax_request=true&server=" +
                serverId;
    ...
    var addOrUpdateSubmenu = function () {
        ...
        $("#edit_user_dialog .submenu-item").each(function () {
            submenuLabel = $(this)
                .find("legend[data-submenu-label]")
                .data("submenu-label");
