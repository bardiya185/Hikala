        ...
        function (url) {
            ...
            $.post(url, params, function (data) {
                ...
                if (typeof data !== "undefined" && data.success === true) {
                    ...
                    $('<div class="sqlqueryresults ajax"></div>').prependTo(
                        "#page_content",
                    );
...
AJAX.registerOnload("table/operations.js", function () {
    ...
    $(document).on("submit", "#copyTable.ajax", function (event) {
        ...
        $.post(
            $form.attr("action"),
            $form.serialize() + argsep + "submit_copy=Go",
            function (data) {
                if (typeof data !== "undefined" && data.success === true) {
                    if (
                        $form
                            .find("input[name='switch_to_new']")
                            .prop("checked")
                    ) {
                        CommonParams.set(
                            "db",
                            $form
                                .find(
                                    "select[name='target_db'],input[name='target_db']",
                                )
                                .val(),
                        );
                        CommonParams.set(
    ...
    $(document).on("submit", "#tableOptionsForm", function (event) {
        ...
        var collationOrigValue = $(
            'select[name="tbl_collation"] option[selected]',
        ).val();
        var $changeAllColumnCollationsCheckBox = $(
            "#checkbox_change_all_collations",
        );
        ...
        if ($tblNameField.val() !== $tblNameField[0].defaultValue) {
            ...
            if (
                $tblCollationField.val() !== collationOrigValue &&
                $changeAllColumnCollationsCheckBox.is(":checked")
            ) {
    ...
    $(document).on(
        "click",
        "#tbl_maintenance li a.maintain_action.ajax",
        function (event) {
            event.preventDefault();
            var $link = $(this);
            ...
            $.post($link.attr("href"), params, function (data) {
                ...
                if (
                    ...
                    var $success = $tempDiv.find(
                        ".result_query .alert-success",
                    );
    ...
    $(document).on("click", "#drop_tbl_anchor.ajax", function (event) {
        ...
        question += Functions.sprintf(
            Messages.strDoYouReally,
            $link[0].getAttribute("data-query"),
        );
        question += Functions.getForeignKeyCheckboxLoader();
