...
TableRelation.setDropdownValues = function ($dropdown, values, selectedValue) {
    ...
    $.each(values, function () {
        optionsAsString +=
            ...
            "'" +
            (selectedValue === Functions.escapeHtml(this)
                ? " selected='selected'"
                : "") +
            ">" +
            Functions.escapeHtml(this) +
...
TableRelation.getDropdownValues = function ($dropdown) {
    ...
    if ($dropdown.is('select[name^="destination_foreign"]')) {
        $databaseDd = $dropdown
            ...
            .parent()
            .parent()
            .find('select[name^="destination_foreign_db"]');
        $tableDd = $dropdown
            .parent()
            .parent()
            .parent()
        ...
        success: function (data) {
            ...
            if (typeof data !== "undefined" && data.success) {
                if (foreignTable === null) {
                    ...
                    var primary = null;
                    if (
                        typeof data.primary !== "undefined" &&
                        1 === data.primary.length
                    ) {
                        primary = data.primary[0];
                    }
                    TableRelation.setDropdownValues(
                        $columnDd.first(),
                        data.columns,
                        primary,
                    );
...
AJAX.registerOnload("table/relation.js", function () {
    ...
    $("body").on("click", "a.add_foreign_key_field", function (event) {
        ...
        $(this)
            ...
            .val("");
        var $sourceElem = $(
            'select[name^="destination_foreign_column[' +
    ...
    $("body").on("click", "a.add_foreign_key", function (event) {
        ...
        $newRow
            .find('select[name^="foreign_key_fields_name"]')
            .not(
                $newRow.find('select[name^="foreign_key_fields_name"]').first(),
            )
            .find('select[name^="destination_foreign_column"]')
            .not(
                $newRow
    ...
    $("a.drop_foreign_key_anchor.ajax").on("click", function (event) {
        ...
        var dropQuery = Functions.escapeHtml(
            $currRow.children("td").children(".drop_foreign_key_msg").val(),
        );
        ...
        $anchor.confirm(question, $anchor.attr("href"), function (url) {
            var $msg = Functions.ajaxShowMessage(
                Messages.strDroppingForeignKey,
                false,
            );
