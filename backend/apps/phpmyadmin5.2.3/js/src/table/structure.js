...
function reloadFieldForm() {
    $.post(
        $("#fieldsForm").attr("action"),
        $("#fieldsForm").serialize() +
        ...
        function (formData) {
            ...
            $("#move_columns_dialog")
                .find("ul")
                .replaceWith($tempDiv.find("#move_columns_dialog ul"));
        },
    );
    $("#page_content").show();
}
...
AJAX.registerOnload("table/structure.js", function () {
            ...
            if (Functions.checkReservedWordColumns($form)) {
                if (checkIfConfirmRequired($form)) {
                    var question = sprintf(
                        Messages.strChangeColumnCollation,
                        "https://wiki.phpmyadmin.net/pma/Garbled_data",
                    );
    ...
    $(document).on("click", "a.drop_column_anchor.ajax", function (event) {
         ...
         */
        var $afterFieldItem = $(
            "select[name='after_field'] option[value='" + currColumnName + "']",
        );
        /**
        ...
        $thisAnchor.confirm(question, $thisAnchor.attr("href"), function (url) {
            var $msg = Functions.ajaxShowMessage(
                Messages.strDroppingColumn,
                false,
            );
            var params = Functions.getJsConfirmCommonParam(
                this,
                $thisAnchor.getPostData(),
            );
            ...
            $.post(url, params, function (data) {
                if (typeof data !== "undefined" && data.success === true) {
                    ...
                    for (
                        var $row = $currRow.next();
                        $row.length > 0;
                        $row = $row.next()
                    ) {
                        var newVal =
                            parseInt($row.find("td").eq(1).text(), 10) - 1;
                        $row.find("td").eq(1).text(newVal);
                    }
                    $afterFieldItem.remove();
                    $currRow.hide("medium").remove();
                    $("select[name=after_field]")
                        .find('[value="' + currColumnName + '"]')
                        .remove();
                    $("select[name=after_field] option")
                        .last()
                    ...
                    Functions.ajaxShowMessage(
                        Messages.strErrorProcessingRequest + " : " + data.error,
                        false,
                    );
                }
            }); // end $.post()
    ...
    $(document).on("click", "a.add_key.ajax", function (event) {
        ...
        var question = Functions.sprintf(
            Messages.strDoYouReally,
            "ALTER TABLE `" +
                Functions.escapeHtml(currTableName) +
                "` " +
                addClause +
                "(`" +
                Functions.escapeHtml(currColumnName) +
                "`);",
        );
    ...
    $(document).on("click", "#move_columns_anchor", function (e) {
        ...
        var colList = $("#move_columns_dialog")
            .find("ul")
            .find("li")
            .remove()
            .end();
        ...
        const designerModalPreviewModal = document.getElementById(
            "designerModalPreviewModal",
        );
        designerModalPreviewModal.addEventListener("shown.bs.modal", () => {
            const modalBody =
                designerModalPreviewModal.querySelector(".modal-body");
            ...
            const formData =
                serialized + sep + "preview_sql=1" + sep + "ajax_request=1";
            $.post({
                url: formUrl,
                data: formData,
                success: (response) => {
    ...
    $(document).on("click", "a[id^=partition_action].ajax", function (e) {
        ...
        if ($link.is("#partition_action_DROP")) {
            $link.confirm(
                Messages.strDropPartitionWarning,
                $link.attr("href"),
                function (url) {
            ...
            $link.confirm(
                Messages.strTruncatePartitionWarning,
                $link.attr("href"),
                function (url) {
                    submitPartitionAction(url);
                },
