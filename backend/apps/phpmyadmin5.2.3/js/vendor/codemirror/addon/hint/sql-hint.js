(function (mod) {
    if (typeof exports == "object" && typeof module == "object")
        ...
        // AMD
        define(["../../lib/codemirror", "../../mode/sql/sql"], mod); // Plain browser env
    else mod(CodeMirror);
})(function (CodeMirror) {
    "use strict";
    ...
    function getModeConf(editor) {
        return (
            editor.getModeAt(editor.getCursor()).config ||
            CodeMirror.resolveMode("text/x-sql")
        );
    }
    ...
    function parseTables(input) {
        ...
        if (isArray(input)) {
            for (var i = input.length - 1; i >= 0; i--) {
                var item = input[i];
                result[getText(item).toUpperCase()] = wrapTable(
                    getText(item),
                    item,
                );
            }
