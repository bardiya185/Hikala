(function (mod) {
    ...
    function clearErrorLines(cm) {
        cm.eachLine(function (line) {
            var has =
                line.wrapClass &&
                /\bCodeMirror-lint-line-\w+\b/.exec(line.wrapClass);
    ...
    function updateLinting(cm, annotationsNotSorted) {
        ...
        for (var line = 0; line < annotations.length; ++line) {
            ...
            anns = anns.filter(function (item) {
                return message.indexOf(item.message) > -1
                    ? false
                ...
                if (ann.to)
                    state.marked.push(
                        cm.markText(ann.from, ann.to, {
                            className:
                                "CodeMirror-lint-mark CodeMirror-lint-mark-" +
                                severity,
                            __annotation: ann,
                        }),
                    );
    ...
    CodeMirror.defineOption("lint", false, function (cm, val, old) {
        ...
        if (val) {
            ...
            if (
                state.options.tooltips != false &&
                state.options.tooltips != "gutter"
            )
                CodeMirror.on(
                    cm.getWrapperElement(),
                    "mouseover",
