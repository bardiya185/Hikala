...
(function ($) {
    ...
    $.jqplot.Highlighter.init = function (target, data, opts) {
        var options = opts || {};
        this.plugins.highlighter = new $.jqplot.Highlighter(
            options.highlighter,
        );
    };
    ...
    $.jqplot.Highlighter.postPlotDraw = function () {
        ...
        this.eventCanvas._elem.before(
            this.plugins.highlighter.highlightCanvas.createElement(
                this._gridPadding,
        ...
        elem = null;
        this.plugins.highlighter._tooltipElem.addClass(
    ...
    function showTooltip(plot, series, neighbor) {
        ...
        if (opts.useAxesFormatters) {
            ...
            var str;
            if (typeof opts.formatString === "string") {
                str = $.jqplot.sprintf.apply(
                    $.jqplot.sprintf,
                    [opts.formatString].concat(neighbor.data),
                );
            } else {
                if (opts.tooltipAxes == "both" || opts.tooltipAxes == "xy") {
                    str =
                        $.jqplot.sprintf(
                            opts.tooltipFormatString,
                            neighbor.data[0],
                        ) +
                        opts.tooltipSeparator +
                        $.jqplot.sprintf(
                            opts.tooltipFormatString,
                            neighbor.data[1],
                        );
        ...
        switch (loc[locationIndicies[opts.tooltipLocation]]) {
            ...
            case "s":
                var x =
                    ...
                    elem.outerWidth(true) / 2;
                var y =
                    gridpos.y + plot._gridPadding.top + opts.tooltipOffset + ms;
                break;
            case "sw":
                var x =
                    gridpos.x +
