...
(function ($) {
    ...
    $.jqplot.PieRenderer.prototype.init = function (options, plot) {
        ...
        this.highlightColorGenerator = new $.jqplot.ColorGenerator(
            this.highlightColors,
        );
    ...
    $.jqplot.PieRenderer.prototype.drawSlice = function (
        ctx,
        ang1,
        ang2,
        color,
    ...
    $.jqplot.PieRenderer.prototype.draw = function (ctx, gd, options, plot) {
        ...
        if (
            options.legendInfo &&
            options.legendInfo.placement == "insideGrid"
        ) {
        ...
        this._center = [
            (cw - trans * offx) / 2 + trans * offx + maxrprime * Math.cos(sa),
            (ch - trans * offy) / 2 + trans * offy + maxrprime * Math.sin(sa),
        ];
        ...
        if (this.shadow) {
            for (var i = 0, l = gd.length; i < l; i++) {
                ...
                this.renderer.drawSlice.call(
                    this,
                    ctx,
                    this._sliceAngles[i][0],
                    this._sliceAngles[i][1],
                    shadowColor,
                    true,
                );
            }
        ...
        for (var i = 0; i < gd.length; i++) {
            ...
            if (this.showSlice[i]) {
                ...
                if (
                    this.showDataLabels &&
                    gd[i][2] * 100 >= this.dataLabelThreshold
                ) {
                    var fstr,
                        avgang =
                            (this._sliceAngles[i][0] +
                    ...
                    var fact =
                        this._radius * this.dataLabelPositionFactor +
                        this.sliceMargin +
                        this.dataLabelNudge;

                    var x =
                        this._center[0] +
                        Math.cos(avgang) * fact +
                        this.canvas._offsets.left;
                    var y =
                        this._center[1] +
                        Math.sin(avgang) * fact +
    ...
    $.jqplot.PieLegendRenderer.prototype.constructor =
    ...
    $.jqplot.PieRenderer.prototype.handleMove = function (
        ev,
        gridpos,
        datapos,
        neighbor,
        ...
        if (neighbor) {
            ...
            if (
                ...
                !(
                    ins[0] == plot.plugins.pieRenderer.highlightedSeriesIndex &&
    ...
    function highlight(plot, sidx, pidx) {
        if (plot.series[sidx].showSlice[pidx]) {
            ...
            s.renderer.drawSlice.call(
                s,
                canvas._ctx,
                s._sliceAngles[pidx][0],
                s._sliceAngles[pidx][1],
                s.highlightColorGenerator.get(pidx),
                false,
            );
        }
    }
