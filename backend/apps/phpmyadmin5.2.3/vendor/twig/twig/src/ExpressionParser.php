...
class ExpressionParser
{
    ...
    {
        $stream = $this->parser->getStream();
        $stream->expect( 9, '[', 'A sequence element was expected');

        $node = new ArrayExpression([], $stream->getCurrent()->getLine());
        $first = true;
        while (!$stream->test( 9, ']')) {
            if (!$first) {
                $stream->expect( 9, ',', 'A sequence element must be followed by a comma');
                if ($stream->test( 9, ']')) {
                    break;
                }
            }
        ...
        $stream->expect( 9, ']', 'An opened sequence is not properly closed');
    ...
    {
        $stream = $this->parser->getStream();
        $stream->expect( 9, '{', 'A mapping element was expected');

        $node = new ArrayExpression([], $stream->getCurrent()->getLine());
        $first = true;
        while (!$stream->test( 9, '}')) {
            if (!$first) {
                $stream->expect( 9, ',', 'A mapping value must be followed by a comma');
                if ($stream->test( 9, '}')) {
                    break;
                }
            }
            ...
            $stream->expect( 9, ':', 'A mapping key must be followed by a colon (:)');
            $value = $this->parseExpression();

            $node->addElement($value, $key);
        }
        $stream->expect( 9, '}', 'An opened mapping is not properly closed');
