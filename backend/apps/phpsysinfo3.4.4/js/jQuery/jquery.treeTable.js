    ...
    jq.fn.jqTreeTable = function (map, options) {
        var opts = jq.extend(
            ...
            buildText = function (parno, preStr) {
                ...
                if (mp !== undefined)
                    for (var y = 0, yl = mp.length; y < yl; y++) {
                        ro = mp[y];
                        if (mapa[ro]) {
                            //It's a parent as well. Build it's string and move on to it's children
                            pre =
                                y == yl - 1 ? opts.blankImg : opts.vertLineImg;
                            img = y == yl - 1 ? opts.lastOpenImg : opts.openImg;
                            mapb[ro - 1] =
                                preStr +
                                '<span class="treeimg" style="background-image: url(\'' +
                                pre +
                            ...
                            pref =
                                preStr +
                                '<span class="treeimg preimg" style="background-image: url(\'' +
                                pre +
                                "')\"></span>";
            ...
            expandKids = function (num, last) {
                ...
                jq("#" + tid + num).css(
                    "background-image",
                    "url('" + (last ? opts.lastOpenImg : opts.openImg) + "')",
                );
            ...
            collapseKids = function (num, last) {
                ...
                if (mapa[num]) {
                    jq("#" + tid + num).css(
                        "background-image",
                        "url('" +
                            (last ? opts.lastShutImg : opts.shutImg) +
                            "')",
                    );
                    for (var x = 0, xl = mapa[num].length; x < xl; x++) {
                        var mnx = mapa[num][x];
            ...
            creset = function (num, exp) {
                ...
                var o = exp
                    ? collarr.splice(jq.inArray(num, collarr), 1)
                    : collarr.push(num);
                cset(tid, collarr);
            },
            cget = function (n) {
        ...
        buildText(0, "");
        jq("tbody tr", this).each(function (i) {
        ...
        if (collarr.length) {
            for (var y = 0, yl = collarr.length; y < yl; y++) {
                collapseKids(
                    collarr[y],
                    $("#" + tid + collarr[y] + ".parimg").attr("last") == 1,
                );
            }
        }
        stripe();
        jq(".parimg", this).each(function (i) {
            ...
            jqt.click(function () {
                ...
                if (jqt.parents("tr").next().is(".collapsed")) {
                    ...
                    //Collapse all and set image to opts.shutImg or opts.lastShutImg on parents
        ...
        if (opts.highlight) {
            //This is where it highlights the rows
            jq("tr", this).hover(
                function () {
