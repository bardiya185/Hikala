...
function PNGload(png) {
    if (typeof png.ifixpng === "function") {
        //IE6 PNG fix
...
function filesystemtable() {
    ...
    html +=
        '          <table id="filesystemTable" style="border-collapse:collapse;">\n';
    html += "            <thead>\n";
    html += "              <tr>\n";
    ...
    if (showTotals) {
        ...
        html +=
            '                <td class="right"><span id="s_fs_tfree"></span></td>\n';
        html +=
            '                <td class="right"><span id="s_fs_tused"></span></td>\n';
        html +=
            '                <td class="right"><span id="s_fs_tsize"></span></td>\n';
        html += "              </tr>\n";
        html += "            </tfoot>\n";
    }
    html += "            <tbody>\n";
...
function createBar(size, barclass, sizeused) {
    ...
    if (percsizeused == percsize) {
        ...
        return (
            ...
            '" style="float:left; width: ' +
            percsizeused +
            'px;">&nbsp;</div><div class="barrest" style="float:left; width: ' +
            (percsize - percsizeused) +
            'px;">&nbsp;</div>&nbsp;' +
            size +
            "%"
        );
...
function refreshVitals(xml) {
    ...
    $("Vitals", xml).each(function getVitals(id) {
        ...
        lastboot = new Date(
            timestamp - parseInt($(this).attr("Uptime"), 10) * 1000,
        );
        ...
        if ($(this).attr("CPULoad") !== undefined) {
            loadavg =
                loadavg +
                "<br>" +
                createBar(parseInt($(this).attr("CPULoad"), 10));
        }
        ...
        setAndStrip(
            ...
            "<img src='./gfx/images/" +
                ...
                "' alt='Icon' title='" +
                icon +
                "' style='width:16px;height:16px;vertical-align:middle;' onload='PNGload($(this));' />&nbsp;" +
                distro,
        ); //onload IE6 PNG fix
        setAndStrip(
            "#s_os",
            "<img src='./gfx/images/" +
                os +
                ".png' alt='OSIcon' title='" +
                os +
                ".png' style='width:16px;height:16px;vertical-align:middle;' onload='PNGload($(this));' />&nbsp;" +
                os,
        ); //onload IE6 PNG fix
...
function fillCpu(xml, tree, rootposition, collapsed) {
    ...
    $("Hardware CPU CpuCore", xml).each(function getCpuCore(cpuCoreId) {
        ...
        if (!isNaN(load) && showCPULoadCompact) {
            html +=
                '<tr><td style="width:68%"><div class="treediv"><span class="treespan">' +
                model +
                "</span></div></td><td>" +
                createBar(load) +
                "</td></tr>\n";
            ...
            html +=
                '<tr><td colspan="2"><div class="treediv"><span class="treespan">' +
                model +
                "</span></div></td></tr>\n";
        }
...
function fillHWDevice(xml, type, tree, rootposition) {
    var devicecount = 0,
        html = "";
    $("Hardware " + type + (type == "MEM" ? " Chip" : " Device"), xml).each(
        function getHWDevice(deviceId) {
            var name = "",
                count = 0,
                capacity = 0,
                manufacturer = "",
...
function refreshHardware(xml) {
    ...
    if (machine !== undefined && machine !== "") {
        html +=
            '    <tr><td colspan="2"><div class="treediv"><span class="treespanbold">' +
            genlang(107) +
            "</span></div></td></tr>\n";
        html +=
            '<tr><td colspan="2"><div class="treediv"><span class="treespan">' +
...
function refreshNetwork(xml) {
    ...
    html1 +=
        '     <th class="right" style="width:17.7%;">' +
        genlang(23) +
        "</th>\n";
    html1 +=
        '     <th class="right" style="width:17.7%;">' +
    ...
    $("Network NetDevice", xml).each(function getDevice(id) {
        ...
        if (showNetworkActiveSpeed) {
            if (rx == 0 && !isNaN(rxr)) {
                if (showNetworkActiveSpeed == 2) {
                    ...
                    htmlrx =
                        "<br><i>(" +
                        formatBytes(round(rxr, 2), xml) +
                        "/s)</i>";
                }
            } else if ($.inArray(name, oldnetwork) >= 0) {
                var diff, difftime;
                if (
                    (diff = rx - oldnetwork[name].rx) > 0 &&
                    (difftime = timestamp - oldnetwork[name].timestamp) > 0
                ) {
                    if (showNetworkActiveSpeed == 2) {
                        htmlrx =
                            "<br><i>(" +
                        ...
                        htmlrx =
                            "<br><i>(" +
                            formatBytes(round(diff / difftime, 2), xml) +
                            "/s)</i>";
                    }
                }
            }
        ...
        if (bridge !== undefined && bridge !== "") {
            html +=
                '<tr><td><div class="treediv"><span class="treespan">' +
                name +
                " (" +
                bridge +
                ')</span></div></td><td class="right">' +
                formatBytes(rx, xml) +
                htmlrx +
                '</td><td class="right">' +
                formatBytes(tx, xml) +
        ...
        if (info !== undefined && info !== "") {
            var i = 0,
                infos = info.replace(/:/g, "<wbr>:").split(";");
            isinfo = true;
            for (i = 0; i < infos.length; i++) {
                html +=
                    '<tr><td colspan="4"><div class="treediv"><span class="treespan">' +
                    infos[i] +
                    "</span></div></td></tr>";
    ...
    if (isinfo) {
        ...
        html0 +=
            '  <table id="NetworkTree" class="stripeMe" style="border-collapse:collapse;">\n';
        html1 += '   <tbody class="tbody_network">\n';
    }
    $("#network").append(html0 + html1 + html);

    if (isinfo)
        $("#NetworkTree").jqTreeTable(tree, {
            openImg: "./gfx/treeTable/tv-collapsable.gif",
            shutImg: "./gfx/treeTable/tv-expandable.gif",
            leafImg: "./gfx/treeTable/tv-item.gif",
            lastOpenImg: "./gfx/treeTable/tv-collapsable-last.gif",
...
function refreshMemory(xml) {
    ...
    html +=
        '      <th class="right" style="width:14.2%;">' +
        genlang(125) +
        "</th>\n";
    html +=
        '      <th class="right" style="width:14.2%;">' +
    ...
    $("Memory", xml).each(function getMemory(id) {
        ...
        html +=
            ...
            createBar(percent) +
            '</td><td class="right">' +
            formatBytes(free, xml) +
            '</td><td class="right">' +
            formatBytes(used, xml) +
            '</td><td class="right">' +
            formatBytes(total, xml) +
            "</td></tr>";
        ...
        $("Memory Details", xml).each(function getMemorydetails(id) {
            ...
            if (!isNaN(buff)) {
                html +=
                    ...
                    "</span></div></td><td>" +
                    createBar(buffp) +
                    '</td><td class="rigth">&nbsp;</td><td class="right">' +
                    formatBytes(buff, xml) +
                    '</td><td class="right">&nbsp;</td></tr>';
                tree.push(memoryindex);
            }
...
function refreshFilesystems(xml) {
    ...
    $("FileSystem Mount", xml).each(function getMount(mid) {
        ...
        if (!isNaN(ignore) && ignore > 0 && showTotals) {
                ...
                if (ignore == 3 && !isNaN(threshold) && percent >= threshold) {
                    filesystemTable.fnAddData([
                        ...
                        '<span style="display:none;">' +
                            percent.toString() +
                            "</span>" +
                            createBar(percent, "barwarn", usage) +
                            inodes_text,
                        '<span style="display:none;">' +
                            free.toString() +
                            "</span><i>(" +
            ...
            if (
                !isNaN(threshold) &&
                percent >= threshold &&
                (showTotals || isNaN(ignore) || ignore < 4)
            ) {
                filesystemTable.fnAddData([
                    '<span style="display:none;">' +
                        mpoint +
                        "</span>" +
                        mpoint,
                    '<span style="display:none;">' + type + "</span>" + type,
                    '<span style="display:none;">' +
                        name +
                        "</span>" +
                        name +
    ...
    if (showTotals) {
        usage =
            total_size != 0 ? Math.ceil((total_used / total_size) * 100) : 0;
        total_usage =
            total_size != 0
                ? round(100 - (total_free / total_size) * 100, 2)
                : 0;
...
function refreshTemp(xml) {
    ...
    $("MBInfo Temperature Item", xml).each(function getTemperatures(id) {
        ...
        if (event !== undefined)
            label +=
                ' <img style="vertical-align: middle; width:16px;" src="./gfx/attention.gif" alt="!" title="' +
                event +
                '"/>';
        limit = parseFloat($(this).attr("Max"));
        if (isFinite(limit)) _limit = formatTemp(limit, xml);
        $("#temperatureTable tbody").append(
            "<tr><td>" +
                label +
                '</td><td class="right">' +
                formatTemp(value, xml) +
                '</td><td class="right">' +
                _limit +
                "</td></tr>",
        );
...
function refreshVoltage(xml) {
    ...
    $("MBInfo Voltage Item", xml).each(function getVoltages(id) {
        ...
        if (isFinite(max)) _max = round(max, 2) + "&nbsp;" + genlang(62);
        min = parseFloat($(this).attr("Min"));
        if (isFinite(min)) _min = round(min, 2) + "&nbsp;" + genlang(62);
        if (isFinite(value))
            $("#voltageTable tbody").append(
                "<tr><td>" +
                    label +
                    '</td><td class="right">' +
                    round(value, 2) +
...
function refreshFans(xml) {
    ...
    $("MBInfo Fans Item", xml).each(function getFans(id) {
        ...
        unit = $(this).attr("Unit");
        if (unit === "%") {
            if (isFinite(min)) _min = round(min, 0) + "%";
            if (isFinite(value))
                $("#fansTable tbody").append(
                    "<tr><td>" +
                        label +
                        "</td><td>" +
                        createBar(round(value, 0)) +
                        '</td><td class="right">' +
                        _min +
                        "</td></tr>",
                );
            ...
            if (isFinite(min)) _min = round(min, 0) + "&nbsp;" + genlang(63);
            if (isFinite(value))
                $("#fansTable tbody").append(
                    "<tr><td>" +
                        label +
    ...
    $("MBInfo Power Item", xml).each(function getPowers(id) {
        ...
        if (event !== undefined)
            label +=
                ...
                '"/>';
        limit = parseFloat($(this).attr("Max"));
        if (isFinite(limit)) _limit = round(limit, 2) + "&nbsp;" + genlang(103);
        if (isFinite(value))
            $("#powerTable tbody").append(
                "<tr><td>" +
                    label +
                    '</td><td class="right">' +
                    round(value, 2) +
    ...
    $("MBInfo Current Item", xml).each(function getCurrents(id) {
        ...
        if (event !== undefined)
            label +=
                ...
                '"/>';
        max = parseFloat($(this).attr("Max"));
        if (isFinite(max)) _max = round(max, 2) + "&nbsp;" + genlang(106);
        min = parseFloat($(this).attr("Min"));
        if (isFinite(min)) _min = round(min, 2) + "&nbsp;" + genlang(106);
        if (isFinite(value))
            $("#currentTable tbody").append(
                "<tr><td>" +
                    label +
                    '</td><td class="right">' +
                    round(value, 2) +
...
function refreshUps(xml) {
    ...
    $("UPSInfo UPS", xml).each(function getUps(id) {
        ...
        battery_charge_percent = parseInt(
            $(this).attr("BatteryChargePercent"),
            10,
        );
        ...
        if (model !== undefined) {
            html +=
                '<tr><td style="width:36%"><div class="treediv"><span class="treespan">' +
                genlang(70) +
                "</span></div></td><td>" +
                model +
                "</td></tr>\n";
    ...
    if (add_apcupsd_cgi_links) {
        html +=
            " (<a title='details' href='/cgi-bin/apcupsd/multimon.cgi' target='apcupsdcgi'>" +
            genlang(99) +
            "</a>)\n";
    }
...
$(document).ready(function buildpage() {
    ...
    showMemoryInfosExpanded =
        $("#showMemoryInfosExpanded").val().toString() === "true";
...
jQuery.fn.dataTableExt.oSort["span-string-desc"] = function sortStringDesc(
    a,
    b,
) {
    var x = "",
...
jQuery.fn.dataTableExt.oSort["span-number-desc"] = function sortNumberDesc(
    a,
    b,
) {
    var x = 0,
...
function buildBlock(plugin, translationid, reload) {
    ...
    if (reload) {
        reloadpic =
            '<img id="Reload_' +
            plugin +
            'Table" src="./gfx/reload.gif" alt="reload" title="reload" style="vertical-align:middle;float:right;cursor:pointer;border:0px;width:16px" />&nbsp;';
    }
    block += '<div id="panel_' + plugin + '" style="display:none;">\n';
    block +=
        '<div id="Plugin_' +
        plugin +
        '" class="plugin" style="display:none;">\n';
