...
function smart_buildTable(xml) {
    ...
    html +=
        '  <table id="Plugin_SMARTTable" style="border-collapse:collapse;">\n';
    html += "    <thead>\n";
    html += "      <tr>\n";
    html += '        <th class="right">' + genlang(2, "SMART") + "</th>\n";
    $("Plugins Plugin_SMART columns column", xml).each(
        function smart_table_header() {
            attribute_name = $(this).attr("attribute_name");
            if (typeof attribute_name === "string")
                attribute_name = attribute_name
                    .replace(/_/g, " ")
                    .replace(/;/g, "<br>");
            else attribute_name = "Attribute " + $(this).attr("id");
...
function smart_populate(xml) {
    ...
    $("Plugins Plugin_SMART disks disk", xml).each(function smart_fill_table() {
        ...
        $(this)
            .find("attribute")
            .each(function smart_fill_data() {
                if (
                    columns[parseInt($(this).attr("id"), 10)] &&
                    columns[parseInt($(this).attr("id"), 10)] !== ""
                ) {
                    values[parseInt($(this).attr("id"), 10)] = $(this).attr(
                        columns[parseInt($(this).attr("id"), 10)],
                    );
                }
            });
        ...
        if (event !== undefined)
            display.push(
                '<span style="display:none;">' +
                    name +
                    '</span><table style="width:0;float:right;*float:right;border-spacing:0;"><tbody><tr><td style="padding:0;">' +
                    name +
                    ' </td><td style="padding:0;"><img style="vertical-align:middle;width:16px;" src="./gfx/attention.gif" alt="!" title="' +
                    event +
                    '"/></td></tr></tbody></table>',
            );
        else
