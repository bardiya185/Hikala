...
elseif(is_string($msgId)) {
	...
	elseif($msgId == "apachemodules") {
		...
		$message['apacheloadedmodules'] = GetApacheLoadedModules($c_apacheVersion, 6, false, $doReport);
		$message_title = "Apache loaded Modules";
		$msg_index = 'apachemodules';
		$complete_result = $message['apacheloadedmodules'];
	}
		...
		if($touche == 'Y') {
			foreach($logToClean as $value) {
				if(file_exists($value)) {
					$fp = fopen($value, "wb");
					fwrite($fp,"--- File cleaned up by Wampserver on ".$date." ---\r\n");
  				fclose($fp);
				}
			}
			if($automaticAll) {
			...
			if(($files = glob($c_phpVersionDir.'/'.$phpVersion.'/zend_ext/php_xdebug-*.dll')) !== false) {
				if(count($files) > 1) {
					...
					if(preg_match('~^;?zend_extension=\"(.*php_xdebug.*)\"\r?$~m',$phpIniContents,$matches) === 1) {
						$zend_ext_PFA_ini = $matches[1];
					}
					$phpIniContents = file_get_contents($c_phpVersionDir.'/'.$phpVersion.'/php.ini');
					if(preg_match('~^;?zend_extension=\"(.*php_xdebug.*)\"\r?$~m',$phpIniContents,$matches) === 1) {
