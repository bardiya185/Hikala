...
if(!in_array($wampConf['LinksChooseIp'],$c_local_ip_list)) {
	...
	$wampConf['LinksChooseIp'] = $c_local_ip;
}
else {
...
if($httpd_error_txt !== '') {
	$WarningsAtEnd = true;
	$message = color('red',$httpd_error_txt);
...
if($wampConf['SupportMySQL'] == 'on' && count($mysqlVersionList) > 0) {
	...
	$mysqlMysqlService = '';
	$mysqlCmdScService = ';';
	$wampIniNewContents['mysqlServiceCmd'] = 'mysqld.exe';
	if(isset($mysqlConf['mysqlServiceCmd']) && $mysqlConf['mysqlServiceCmd'] == 'windows') {
		$mysqlMysqlService = ';';
		$mysqlCmdScService = '';
		$wampIniNewContents['mysqlServiceCmd'] = 'windows';
	}
	wampIniSet($configurationFile, $wampIniNewContents);
}
else {
...
if($wampConf['SupportMariaDB'] == 'on' && count($mariadbVersionList) > 0) {
	...
	$mariaMysqlService = '';
	$mariaCmdScService = ';';
	$wampIniNewContents['mariadbServiceCmd'] = 'mysqld.exe';
	if(isset($mariadbConf['mariadbServiceCmd']) && $mariadbConf['mariadbServiceCmd'] == 'windows') {
		$mariaMysqlService = ';';
		$mariaCmdScService = '';
		$wampIniNewContents['mariadbServiceCmd'] = 'windows';
	}
	wampIniSet($configurationFile, $wampIniNewContents);
}
else {
	...
	if($wampConf['NotVerifyHosts'] == 'off') {
		...
		if($rewriteHost) {
			...
			if($fp !== false) {
				...
				$errorTxt = 'Unable to open '.$c_hostsFile.' file';
			  error_log($errorTxt);
 				if(WAMPTRACE_PROCESS) error_log("script ".__FILE__."\n*** ".$errorTxt."\n",3,WAMPTRACE_FILE);
				  if($doReport) {
				  	if(!$localIPv4)
				  	...
				  	if(!$localIPv6)
				  		$wampReport['gen2'] .= "\n-- No ::1 localhost in hosts file";
				  	$wampReport['gen2'] .="\n- Unable to open ".$c_hostsFile." file";
				  }
			}
			fclose($fp);
		}
	...
	foreach($logFilesSizeClean as $key => $value) {
		if($value > 100000) {
			if($with_file) {
				...
				$command = 'find /v /c "" '.$key;
				$output = shell_exec($command);
				if(preg_match('~^[ \t-]+'.$key.':[ \t]+([0-9]+\r?$)~mi',$output,$matches) === 1) {
					if($matches[1] > $wampConf['AutoCleanLogsMax']) {
						$command = 'powershell -command "& {Get-Content '.$key.' | Select-Object -Last '.$wampConf['AutoCleanLogsMin'].'}" >'.$c_installDir.'/'.$logDir.'tempfile.txt';
						$output = shell_exec($command);
						if(empty($output)) {
							copy($c_installDir.'/'.$logDir.'tempfile.txt',$key);
							unlink($c_installDir.'/'.$logDir.'tempfile.txt');
						}
					}
				}
			}
...
unset($logFilesSizeClean,$value);
if(WAMPTRACE_PROCESS) {
	$errorTxt = ' - End of log file clean-up';
	require_once 'start_time.php';
	$errorTxt .= ' - Mem='.memory_get_peak_usage(true).' - Elapsed time='.(microtime(true)-$start_time);
	error_log("script ".__FILE__.$errorTxt."\n",3,WAMPTRACE_FILE);
}
...
foreach($AesTextMenus as $key => $value) {
	...
	$TextMenus .= 'TextKey'.$key.'="'.$CaptionTemp.'",'.$value[2].','.$value[3].','.$value[4].','.$value[5].','.$value[6].',';
	$TextMenus .= $value[7].','; //Flash, RTF or PDF
	if(strpos($value[8],'$') === 0) {
		$temp = substr($value[8],1);
		$tempText = trim($$temp);
	}
	else {
		$tempText = $value[8];
	...
	if($value[7] == 'Flash'){
		if(is_array($value[9])) {
			...
			if(strpos($value[9],'$') === 0 ){
				$temp = substr($value[9],1);
				$tempText = trim($$temp);
			}
			else
				$tempText = $value[9];
		}
		if($value[10] > 0) $tempText = wordwrap($tempText,$value[10],"\r\n");
		$tempText = ReplaceAestan($tempText);
		$TextMenus .= $tempText."\r\n";
		$Glyph[] = ($value[11] > -1) ? '; Glyph: '.$value[11] : '';
	}//End Flash
	else { //PDF or RTF
		$file_dir = pathinfo($value[9],PATHINFO_DIRNAME);
		$file_name = pathinfo($value[9],PATHINFO_FILENAME);
		$file_ext = '.'.pathinfo($value[9],PATHINFO_EXTENSION);
		if(strpos($file_name,'english') !== false) {
			$file_name = str_replace('english','',$file_name);
			$file = $c_installDir.'/'.$file_dir.'/'.$file_name.$lang.$file_ext;
			if(!file_exists($file)) {
				$file = $c_installDir.'/'.$file_dir.'/'.$file_name.'english'.$file_ext;
			}
		}
		else {
			$file = $c_installDir.'/'.$file_dir.'/'.$file_name.$file_ext;
		}
		$file = str_replace('/','\\',$file);
		$Glyph[] = ($value[10] > -1) ? '; Glyph: '.$value[10] : '';
...
foreach($TextSubmenuName as $value) {
	...
	$TextSubmenus .= <<< EOF
[{$value}]
Type: item; Caption: "{$TextSubmenuCaption[$i]}"; Action: TextKey{$GlyphM}
...
foreach ($langList as $langname => $langstatus) {
  ...
  $langText .= <<< EOF
[lang_{$cleanLangName}]
Action: run; FileName: "{$c_phpExe}";Parameters: "changeLanguage.php {$cleanLangName}";WorkingDir: "{$c_installDir}/scripts"; Flags: waituntilterminated; ShowCmd: hidden
Action: multi; Actions: refresh_readconfig; Flags:appendsection
...
';
  if($incompatiblePhp == 0) {
  	$clean = ($wampConf['CleanLogs_SwitchVersion'] == 'on') ? '' : ';';
  	$myreplacemenu .= <<< EOF
[switchPhp{$onePhpVersion}]
{$clean}Action: run; FileName: "{$c_phpExe}"; parameters: "msg.php refreshLogs alllogs automatic";WorkingDir: "$c_installDir/scripts"; Flags: waituntilterminated; ShowCmd: hidden
Action: run; FileName: "{$c_phpExe}";Parameters: "switchPhpVersion.php {$onePhpVersion}";WorkingDir: "{$c_installDir}/scripts"; Flags: waituntilterminated; ShowCmd: hidden
...
foreach($PHP_List_Versions as $php_version_value) {
	...
	if(!$PHP_Apache_Module && $php_version_value == $c_phpVersion) {
		...
		$PHP_version = $PHP_version_space = $c_phpVersion;
...
EOF;
	...
	foreach ($ext as $extname=>$extstatus){
		$PHP_extensions[$PHP_version][$extname] = $extstatus;
	  if($ext[$extname] == 1) {
	  	$NBextPHPlines++;
...
Action: run; FileName: "{$c_phpExe}";Parameters: "switchPhpExt.php {$extname} {$SwitchAction} {$PHP_version}";WorkingDir: "{$c_installDir}/scripts"; Flags: waituntilterminated; ShowCmd: hidden
...
EOF;
		...
		elseif($ext[$extname] == -4) {
			...
			$extText .= <<< EOF
[php_ext_{$PHP_version}{$extname}]
Action: run; FileName: "{$c_phpExe}";Parameters: "switchPhpExt.php {$extcontent} {$SwitchAction} {$PHP_version}";WorkingDir: "{$c_installDir}/scripts"; Flags: waituntilterminated; ShowCmd: hidden
Action: multi; Actions: {$PHP_Restart}; Flags:appendsection
...
Action: run; FileName: "'.$c_phpExe.'";Parameters: "msg.php '.$msgNum.' '.base64_encode($extname_msg).'";WorkingDir: "'.$c_installDir.'/scripts"; Flags: waituntilterminated; ShowCmd: normal
';
		}
	}
...
foreach($PHP_List_Versions as $php_version_value) {
	...
	if(!$PHP_Apache_Module && $php_version_value == $c_phpVersion) {
		...
		elseif($php_version_value == $c_phpCliVersion) {
			$PHP_Wamp_CLI = true;
			$phpParams = $phpParamsCLI;
			$PHP_FCGI_Mode = true;
			$PHP_ini_File = $c_phpVersionDir.'/php'.$php_version_value.'/php.ini';
			$PHP_ext_Dir = $c_phpVersionDir.'/php'.$php_version_value.'/ext/';
			$PHP_version = 'CLI'.$php_version_value;
			$PHP_version_space = '[CLI] '.$php_version_value;
			$PHP_extSubmenu = true;
			$PHP_Restart= 'refresh_readconfig';
		}
		else {
...
EOF;
$phpConfTextInfo = $phpConfTextComment = "";
...
Action: run; FileName: "{$c_phpExe}";Parameters: "changePhpParam.php {$quoted} {$action} {$tz} none {$PHP_version}";WorkingDir: "{$c_installDir}/scripts"; Flags: waituntilterminated; ShowCmd: hidden
Action: multi; Actions: {$PHP_Restart}; Flags:appendsection
...
Action: run; FileName: "{$c_phpExe}";Parameters: "changePhpParam.php {$quoted} {$action} {$param_value} {$param_third} {$PHP_version}";WorkingDir: "{$c_installDir}/scripts"; Flags: waituntilterminated; ShowCmd: hidden
...
Action: run; FileName: "{$c_phpExe}";Parameters: "switchPhpParam.php {$phpParams[$paramname]} {$SwitchAction} {$PHP_version}";WorkingDir: "{$c_installDir}/scripts"; Flags: waituntilterminated; ShowCmd: hidden
{$Apache_Restart_Php_Conf}
...
if($phmyadOK && empty($SupportDBMS)) {
	...
	$numPhpMy = 0;
	foreach($Alias_Contents['PMyAdVer'] as $key => $PMA_version) {
		if($numPhpMy > 0) {
			$ItemMenuPMA .= <<< EOF
Type: separator
...
EOF;
		...
		if(stripos($value,'phpmyadmin') === false && stripos($value,'adminer') === false) continue;
		$glyph_app = (stripos($value,'adminer') !== false) ? 44 : 39;
		$glyph = ($Alias_Contents[$value]['compat']) ? $glyph_app : 23;
		$ItemMenuPMA .= <<< EOF
...
EOF;
		if(stripos($value,'adminer') === false && version_compare($Alias_Contents[$value]['version'],'5.2.1','>')) {
			...
			$PMA_ver = $PMA_version;
			if(isset($Alias_Contents[$PMA_version]) === false) $PMA_ver = 'phpmyadmin';
			if($Alias_Contents[$PMA_ver]['fcgid'] > 0) $PHP_Version_used = 'FCGI'.$Alias_Contents[$PMA_ver]['fcgidPHP'];
			if(isset($PHP_extensions[$PHP_Version_used]['sodium']) && $PHP_extensions[$PHP_Version_used]['sodium'] === '0') {
				$message = 'The PHP extension "sodium" must be loaded before this version of PhpMyAdmin can be launched.
';
				$ItemMenuPMA .= <<< EOF
Type: item; Caption: "{$w_ExtNotLoaded} sodium"; Action: multi; Actions:phpmyadmin_no_ext_sodium; Glyph: 19

EOF;
				$SubMenuPMA .= '
[phpmyadmin_no_ext_sodium]
...
';
		...
		if($Alias_Contents[$value]['hide'] !== 'none') {
			$Glyph = ''; $Action = 'hidedb';
...
Action: run; FileName: "{$c_phpExe}";Parameters: "changeMiscParam.php phpmyadmin cfghidedb {$Alias_Contents[$value]['path']}/config.inc.php {$Action}";WorkingDir: "{$c_installDir}/scripts"; Flags: waituntilterminated; ShowCmd: hidden
Action: multi; Actions: refresh_readconfig; Flags:appendsection

EOF;
		}
		if($Alias_Contents[$value]['nopassword'] !== 'none') {
...
Action: run; FileName: "{$c_phpExe}";Parameters: "changeMiscParam.php phpmyadmin cfgpassword {$Alias_Contents[$value]['path']}/config.inc.php {$Action}";WorkingDir: "{$c_installDir}/scripts"; Flags: waituntilterminated; ShowCmd: hidden
Action: multi; Actions: refresh_readconfig; Flags:appendsection
...
EOF;
			  		...
			  		if($PMA_ParamsNotOnOff[$next_param_name]['change'] !== true) {
			  	  	...
			  	  	if(!empty($PMA_ParamsNotOnOff[$next_param_name]['msg'])) {
			  	  		$PMAErrorMsg[$next_param_name] = "\n".$PMA_ParamsNotOnOff[$next_param_name]['msg']."\n";
								...
								$PMAErrorMsg[$next_param_name] = "\nThe value of this PhpMyAdmin parameter must be modified in the file: ".$Alias_Contents[$value]['aliaspath']."\n";
							}
			  		}
			  		else {
			  	  $params_for_PMA_alias[$next_param_name] = -3;
			  	  if(!empty($PMA_ParamsNotOnOff[$next_param_name]['same'])) {
			  	  	$params_for_PMA_alias[$next_param_name] = -5;
			  	  	$params_same[$next_param_name] = $PMA_ParamsNotOnOff[$next_param_name]['same'];
			  	  }
			...
			$action_sup = $action_sup_code = array();
			$information_only = false;
...
';
				...
				elseif($params_for_PMA_alias[$paramname] == -2) { // Indicate different from 0 or 1 or On or Off
					if(!$information_only) {
...
';
				...
				elseif($params_for_PMA_alias[$paramname] == -3 || $params_for_PMA_alias[$paramname] == -5) { // Indicate different from 0 or 1 or On or Off but can be changed
					$action_sup[] = $paramname;
					$action_sup_code[] = '';
...
';
					if($params_for_PMA_alias[$paramname] == -5) {
						$action_sup_code[] = 'same';
					}
				}
...
EOF;
							...
							$SubMenuSup[$i] .= <<< EOF
[{$action}{$value}{$PhpMyAdminVersion}]
Action: run; FileName: "{$c_phpExe}";Parameters: "changeMiscParam.php phpmyadmin aliasvalue {$Alias_Path} {$quoted} {$new_action} {$param_value}{$param_third}";WorkingDir: "{$c_installDir}/scripts"; Flags: waituntilterminated; ShowCmd: hidden
...
Action: run; FileName: "{$c_phpExe}";Parameters: "changeMiscParam.php phpmyadmin aliasflag {$Alias_Path} {$PMAParams[$paramname]} {$SwitchAction}";WorkingDir: "{$c_installDir}/scripts"; Flags: waituntilterminated; ShowCmd: hidden
Action: multi; Actions: refresh_readconfig; Flags:appendsection
...
$ItemMenuPMA .= <<< EOF
Type: separator
Type: item; Caption: "{$w_phpMyAdminHelp}"; Action: TextKey; Glyph: 35
...
foreach($Alias_Contents['alias'] as $Alias_Value) {
	if($Alias_Contents[$Alias_Value]['fcgid'] && array_key_exists('php_admin_value',$Alias_Contents[$Alias_Value])){
		...
		$phpfcgi_options = '';
		foreach($Alias_Contents[$Alias_Value]['php_admin_value'] as $key => $php_admin_value) {
			$phpfcgi_options .= ' -d '.$php_admin_value['param'].'='.$php_admin_value['value'];
		}
		$Alias_PMA_Contents = file_get_contents($Alias_Path);
		$count = $counts = 0;
		$write_file = false;
		if(stripos($Alias_PMA_Contents, '${FCGIPHPOPTIONS}') !== false) {
			...
			$Alias_PMA_Contents = preg_replace('{^([ \t]*)(<Files.+>)[ \t]*\r?$}mi',"$1Define FCGIPHPOPTIONS \"".$phpfcgi_options."\"\r\n$0",$Alias_PMA_Contents,-1,$count);
			$counts +=$count;
			$search = 'FcgidWrapper "${PHPROOT}${FCGIPHPVERSION}/php-cgi.exe';
			$Alias_PMA_Contents = str_replace($search,$search.'${FCGIPHPOPTIONS}',$Alias_PMA_Contents,$count);
			$counts += $count;
			if($counts == 2) $write_file = true;
		}
		if($write_file) {
			write_file($Alias_Path,$Alias_PMA_Contents);
		}
	}
}
if(WAMPTRACE_PROCESS) {
...
Action: run; FileName: "{$c_phpExe}";Parameters: "switchApacheVersion.php {$oneApacheVersion}";WorkingDir: "{$c_installDir}/scripts"; Flags: waituntilterminated; ShowCmd: normal
Action: run; FileName: "{$c_phpExe}";Parameters: "switchPhpVersion.php {$wampConf['phpVersion']}";WorkingDir: "{$c_installDir}/scripts"; Flags: waituntilterminated; ShowCmd: hidden
...
Action: run; FileName: "{$c_phpExe}";Parameters: "switchWampPort.php {$c_UsedPort}";WorkingDir: "{$c_installDir}/scripts"; Flags: waituntilterminated; ShowCmd: hidden
...
foreach ($mod as $modname => $modstatus) {
    ...
    $httpdText .= <<< EOF
[apache_mod_{$modname}]
Action: run; FileName: "{$c_phpExe}";Parameters: "switchApacheMod.php {$modname} {$SwitchAction}";WorkingDir: "{$c_installDir}/scripts"; Flags: waituntilterminated; ShowCmd: hidden
...
Type: item; Caption: "'.$w_apache_compare.'"; Action: TextKey; Glyph: 22
...
';
		$httpdTextMenu .= <<< EOF
[apache_comp_{$value}]
Action: run; FileName: "{$c_phpExe}";Parameters: "switchApacheVersion.php {$c_apacheVersion} {$value} compare";WorkingDir: "{$c_installDir}/scripts"; Flags: waituntilterminated; ShowCmd: normal
Action: multi; Actions: apache_restart_refresh; Flags:appendsection
...
Type: item; Caption: "'.$w_apache_restore.'"; Action: TextKey; Glyph: 22
...
";
$httpdTextMod = $httpdTextSub = $httpdTextSubMenu = $httpdTextSubMenuTxt = '';
$httpdConfParams = $httpdConfParamsValue = $search = $replace = array();
$ApacheDefaultOnlyTxt = '';
$ApacheDefaultTitle = false;
$i=-1;
foreach($apache_Params as $key => $value ) {
	...
	if(preg_match('~^[ \t]*(#?)[ \t]*('.$key.')[ \t]+('.$value['mask'].')\r?$~mi',$myhttpdContents,$matches) > 0) {
		...
		if($matches[1] == '#') {
			$param_commented = true;
			$httpdConfParams[++$i][$key] = '#'.$matches[3];
		}
		if(!$param_commented) {
			$httpdConfParams[++$i][$key] = $matches[3];
			if(array_key_exists('value',$apache_Params[$key])) {
				$httpdConfParamsValue[$key] = $apache_Params[$key]['value'];
			}
		}
		else {
			if(!$ApacheDefaultTitle) {
				$ApacheDefaultOnlyTxt .= 'Type: separator; Caption: "'.$w_apacheDefaults.'"
';
				$ApacheDefaultTitle = true;
			}
		$ApacheDefaultOnlyTxt .= "Type: item; Caption: \"".$key."  ".$apache_Params[$key]['default']."\"; Action: none
";
		}
		if(array_key_exists('max',$apache_Params[$key])) {
			$depends = $apache_Params[$key]['max'];
			$depends_value = 'none';
			if(array_key_exists($depends,$httpdParams)) $depends_value = $httpdParams[$depends];
			$httpdConfParams[$i][$key] .= '^'.$depends.'^'.$depends_value;
		}
		if(array_key_exists('value',$apache_Params[$key])) {
			$httpdConfParamsValue[$key] = $apache_Params[$key]['value'];
		}
	...
	foreach($value as $key1 => $value1) {
		if(stripos($value1,'^') !== false ){
			...
			if($temp[0] > $temp[2]) {
				...
				$message .= " The value of '".$temp[1]."' must be equal to or greater than that of '".$key1."'\n\n";
...
';
		...
		elseif($value1 == '>>>ERROR<<<') {
			...
			$message .= " Accepted values are:\n".implode("\n",explode('|',$apache_Params[$key1]['mask']))."\n";
			$httpdText .= 'Type: item; Caption: "'.$key1.' '.$value1.'"; Glyph: 19; Action: run; FileName: "'.$c_phpExe.'";Parameters: "msg.php 11 '.base64_encode($message).'";WorkingDir: "'.$c_installDir.'/scripts"; Flags: waituntilterminated
';
			$treated = true;
		}
		if($treated === false) {
			...
			if($commented) {
				$httpdText .= <<< EOF
Type: submenu; Caption: "{$keyaff}"; Submenu : uncomment{$keyaff}; Glyph: 34

EOF;
				$httpdTextSub .= <<< EOF
[uncomment{$keyaff}]
Type: item; Caption: "{$w_uncomment}"; Action: multi; Actions: uncommentgo{$keyaff}; Glyph: 34

[uncommentgo{$keyaff}]
Action: run; FileName: "{$c_phpExe}";Parameters: "changeApacheParam.php {$key1} {$value1} none uncomment";WorkingDir: "{$c_installDir}/scripts"; Flags: waituntilterminated; ShowCmd: hidden
Action: multi; Actions: apache_stop_start_refresh; Flags:appendsection

EOF;
			}
			elseif(array_key_exists($key1,$httpdConfParamsValue)) {
				$httpdText .= <<< EOF
Type: submenu; Caption: "{$keyaff} {$value1}"; Submenu : changeparam{$keyaff}; Glyph: 9

EOF;
				$httpdTextSubMenu .= '[changeparam'.$keyaff.']
Type: separator; Caption: "Size"
';
				foreach($httpdConfParamsValue[$key1] as $newvalue) {
					if($newvalue <> $value1) {
						$httpdTextSubMenu .= <<< EOF
Type: item; Caption: "{$newvalue}"; Action: multi; Actions: changeparamgo{$keyaff}

EOF;
						$httpdTextSubMenuTxt .= <<< EOF
[changeparamgo{$keyaff}]
Action: run; FileName: "{$c_phpExe}";Parameters: "changeApacheParam.php {$key1} {$value1} {$newvalue} change";WorkingDir: "{$c_installDir}/scripts"; Flags: waituntilterminated; ShowCmd: hidden
Action: multi; Actions: apache_stop_start_refresh; Flags:appendsection
...
EOF;
				...
				$httpdText .= "Type: item; Caption: \"".$keyaff." ".$value1."\"; Action: none
";
			}
		}
	}
...
foreach ($aliasDirContents as $one_alias)
{
  ...
  $myreplace .= 'Type: submenu; Caption: "'.$localhostHTTP.'://localhost/'.$newalias_dir.'/"; SubMenu: alias_'.$newalias_dir.'; Glyph: 3
';
...
EOF;
		$DBMSFooter = <<< EOF
Type: item; Caption: "{$w_MariaDBMySQLHelp}"; Action: TextKey; Glyph: 31
...
Action: run; FileName: "{$c_phpExe}"; Parameters: "switchMariaPort.php 3307 nocheck";WorkingDir: "{$c_installDir}/scripts"; Flags: waituntilterminated; ShowCmd: hidden
Action: run; FileName: "{$c_phpExe}"; Parameters: "switchMysqlPort.php 3306 nocheck";WorkingDir: "{$c_installDir}/scripts"; Flags: waituntilterminated; ShowCmd: hidden
...
Action: service; Service: {$c_mysqlService}; ServiceAction: stop; Flags: ignoreerrors waituntilterminated
Action: service; Service: {$c_mariadbService}; ServiceAction: stop; Flags: ignoreerrors waituntilterminated
Action: run; FileName: "{$c_phpExe}"; Parameters: "switchMysqlPort.php 3308 nocheck";WorkingDir: "{$c_installDir}/scripts"; Flags: waituntilterminated; ShowCmd: hidden
Action: run; FileName: "{$c_phpExe}"; Parameters: "switchMariaPort.php 3306 nocheck";WorkingDir: "{$c_installDir}/scripts"; Flags: waituntilterminated; ShowCmd: hidden
Action: service; Service: {$c_mariadbService}; ServiceAction: startresume; Flags: ignoreerrors waituntilterminated
Action: service; Service: {$c_mysqlService}; ServiceAction: startresume; Flags: ignoreerrors waituntilterminated
Action: multi; Actions: apache_restart_refresh; Flags:appendsection
EOF;
...
";
	...
	if(count($Alias_Contents['alias']) > 0)	{
		foreach($Alias_Contents['alias'] as $AliasValue) {
			if(stripos($AliasValue,'phpmyadmin') !== false || stripos($AliasValue,'adminer') !== false) continue;
			...
			$myreplacesubmenuAlias .= $localhostHTTP.'://localhost'.$UrlPort.'/'.$Alias_Contents[$AliasValue]['alias'].'/"; Glyph: 5
';
		}
...
Action: run; FileName: "'.$c_phpExe.'";Parameters: "msg.php 14";WorkingDir: "'.$c_installDir.'/scripts"; Flags: waituntilterminated; ShowCmd: normal
';
}
else {
...
';
						...
						$value_link = $value;
						$vh_action = 'Action: run; FileName: "'.$c_navigator.'"; Parameters: "http://'.$value_url.$UrlPortVH.'/"; Glyph: '.$glyph;
						$server_name[$value] = 1;
						$value_type = $vh_ip = '';
						if($virtualHost['ServerNameIp'][$value] !== false) {
							$vh_ip = $virtualHost['ServerNameIp'][$value];
							$value_link = $vh_ip.' ('.$value.')';
							$vh_action = 'Action: run; FileName: "'.$c_navigator.'"; Parameters: "http://'.$vh_ip.$UrlPortVH.'/"; Glyph: '.$glyph;
						...
						if(in_array($value,$virtualHost['ServerNameHttps'])) {
							...
							if(!empty($UrlPortVH)) $vh_action = str_replace($UrlPortVH,'',$vh_action);
						}
						$myreplacesubmenuVhosts .= 'Type: item; Caption: "'.$value_link.$value_type.'"; '.$vh_action.'
';					if($key != 'localhost' || $wampConf['AllowLocalhostHttps'] == 'on') {
...
[{$nameHttps}https]
Action: run; FileName: "{$c_phpExe}";Parameters: "changeToHttps.php https {$nameHttps} {$docrootHttps} {$ipHttps} {$fcgiHttps}";WorkingDir: "{$c_installDir}/scripts"; Flags: waituntilterminated; ShowCmd: normal
Action: multi; Actions: apache_stop_start_refresh; Flags:appendsection
...
';
						$myreplacesubmenuHttps .= <<<EOF
[{$nameHttps}nohttps]
Action: run; FileName: "{$c_phpExe}";Parameters: "changeToHttps.php nohttps {$nameHttps} {$docrootHttps} {$ipHttps} {$fcgiHttps}";WorkingDir: "{$c_installDir}/scripts"; Flags: waituntilterminated; ShowCmd: normal
Action: multi; Actions: apache_stop_start_refresh; Flags:appendsection
...
if(array_key_exists('localhost',$server_name) !== true) {
	...
	$WarningsAtEnd = true;
	$message1 = color('red',$message)."The VirtualHost 'localhost' existed originally and needs to be put back.\r\n\r\nA copy of the original httpd-vhosts.conf file exists in:\r\n {$c_apacheConfDir}/original/wampserver\r\n";
	$WarningText .= 'Type: item; Caption: "localhost does not exist"; Glyph: 19; Action: run; FileName: "'.$c_phpExe.'";Parameters: "msg.php 11 '.base64_encode($message1).'";WorkingDir: "'.$c_installDir.'/scripts"; Flags: waituntilterminated
';
}
else {
	if($wampConf['CheckHttpdRequire'] == 'on') {
		if(!empty($virtualHost['ServerNameRequire']['localhost'])) {
			if(strpos($virtualHost['ServerNameRequire']['localhost'],'local') === false) {
				$WarningsAtEnd = true;
				$message = color('red',"VirtulaHost localhost don't use Require local\r\n")."The VirtualHost 'localhost' must use 'Require local' directive.\r\n";
				$WarningText .= 'Type: item; Caption: "localhost must use Require local"; Glyph: 19; Action: run; FileName: "'.$c_phpExe.'";Parameters: "msg.php 11 '.base64_encode($message).'";WorkingDir: "'.$c_installDir.'/scripts"; Flags: waituntilterminated
';
			}
		}
	}
...
$params_for_wampconf = $wampConfParams = $showcmd = array();
foreach($wamp_Param as $value) {
	$value_temp = str_replace(array('#','-'),'',$value);
	if(!empty($wampParamsNotOnOff[$value_temp]['seeifconf']) && $wampConf[$wampParamsNotOnOff[$value_temp]['seeifconf']] !== 'on') {
		continue;
	}
	$endsub = false;
  ...
  elseif(empty($wampConf[$value])) {//Parameter does not exist in wampmanager.conf
	...
	elseif(array_key_exists($value, $wampParamsNotOnOff)) {
		if(!empty($wampParamsNotOnOff[$value]['showcmd'])) $showcmd[$value] = $wampParamsNotOnOff[$value]['showcmd'];
...
Action: run; FileName: "{$c_phpExe}";Parameters: "switchWampParam.php {$wampConfParams[$paramname]} {$SwitchAction}";WorkingDir: "{$c_installDir}/scripts"; Flags: waituntilterminated; ShowCmd: {$showcmd[$paramname]}
Action: multi; Actions: apache_restart_refresh; Flags:appendsection
...
';
			...
			$SubMenuSup[$i] .= <<< EOF
[{$action}{$value}]
Action: run; FileName: "{$c_phpExe}";Parameters: "changeWampParam.php {$quoted} {$action} {$param_value}{$param_third}";WorkingDir: "{$c_installDir}/scripts"; Flags: waituntilterminated; ShowCmd: normal
Action: multi; Actions: apache_restart_refresh; Flags:appendsection
...
';
		include 'ListBrowsers.php';
			...
			foreach($browser['name'] as $key => $value) {
				...
				$BrowserChangeMenu .= 'Type: Item; Caption: "'.$browserName.'"; Action: multi; Actions: browser_'.$browserName.$glyph.'
	';
				$BrowserChangeSub .= <<< EOF
[browser_{$browserName}]
Action: run; FileName: "{$c_phpExe}";Parameters: "changeWampParam.php quotes navigator ""{$browserPath}""";WorkingDir: "{$c_installDir}/scripts"; Flags: waituntilterminated; ShowCmd: hidden
Action: multi; Actions: refresh_readconfig; Flags:appendsection
...
Action: run; FileName: "{$c_phpExe}";Parameters: "deleteVersion.php {$appli} {$appliVersion}";WorkingDir: "{$c_installDir}/scripts"; Flags: waituntilterminated; ShowCmd: hidden
Action: multi; Actions: refresh_readconfig; Flags:appendsection
...
Action: run; FileName: "{$c_phpExe}";Parameters: "ListenPortApache.php delete {$ListenPort}";WorkingDir: "{$c_installDir}/scripts"; Flags: waituntilterminated; ShowCmd: normal
Action: multi; Actions: apache_restart_refresh; Flags:appendsection
...
if($doReport) {
	...
	unset($value);
	$wampReportTxt .= "\n--------- wampmanager.ini (Last 5 lines) --------\n";
	$wampReportTxt .= implode(PHP_EOL, array_slice(file($wampserverIniFile), -5));
...
unset($tpl);
if(WAMPTRACE_PROCESS) {
	$errorTxt = ' - End of create wampmanager.ini file from template file';
	require_once 'start_time.php';
	$errorTxt .= ' - Mem='.memory_get_peak_usage(true).' - Elapsed time='.(microtime(true)-$start_time);
	error_log("script ".__FILE__.$errorTxt."\n",3,WAMPTRACE_FILE);
}
...
if(WAMPTRACE_PROCESS) {
	$errorTxt = ' - End of the script refresh.php';
	require_once 'start_time.php';
	$errorTxt .= ' - Mem='.memory_get_peak_usage(true).' - Elapsed time='.(microtime(true)-$start_time);
	error_log("script ".__FILE__.$errorTxt."\n",3,WAMPTRACE_FILE);
}
