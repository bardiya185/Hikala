...
EOFFCGIPHP;
	...
	elseif(strip_tags(trim($_POST['addmodify'])) == 'modifyalias') { // Modify Alias
		$AliasModified = false;
		$vh_name = strip_tags(trim($_POST['vh_name']));
		if(!in_array($vh_name,$Alias_Contents['alias'],true)) exit;
		$AliasToMod = json_decode($_POST['modifycontent'],true);
		$AliasToMod['path'] = $Alias_Contents[$vh_name]['aliaspath'];
		$AliasToMod['original'] = file_get_contents($AliasToMod['path']);
		$vh_fcgi_on = false;
		$vh_fcgi_php = '';
		if(isset($_POST['vh_fcgi_on']) && strip_tags(trim($_POST['vh_fcgi_on'])) == 'on') {
			$vh_fcgi_on = true;
  ...
  <div id="head">
    <div class="innerhead">
	    <h1><abbr title="Windows">W</abbr><abbr title="Apache">a</abbr><abbr title="MySQL/MariaDB">m</abbr><abbr title="PHP">p</abbr><abbr title="server WEB local">server</abbr></h1>
		   <ul>
			   <li>Apache 2.4</li><li>-</li><li>MySQL 5, 8 &amp; 9</li><li>-</li><li>MariaDB 10 &amp; 11</li><li>-</li><li>PHP 5, 7 &amp; 8</li>
		   </ul>
    </div>
		<ul class="utility">
