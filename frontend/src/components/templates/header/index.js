...
function Header() {
  ...
  return (
    <>
      <header dir="ltr" className="w-full bg-white font-sans select-none">
        {/* Banner */}
        <div className="w-full overflow-hidden">
          <Image
            ...
            alt="banner"
            priority
            className="block w-full h-[45px] sm:h-[50px] md:h-[60px] object-cover"
          />
        </div>

        {/* MAIN HEADER */}
        <div className="w-full bg-white font-sans select-none px-3 sm:px-4 lg:px-6 mt-3 sm:mt-4">
          <div className="flex items-center justify-between gap-2 sm:gap-3 lg:gap-6 w-full">
            <div className="flex w-full items-center justify-between gap-2 sm:gap-3 lg:gap-6 flex-1 min-w-0">
              {/* Logo */}
              <Link
                href="/"
            ...
            <div className="hidden lg:flex items-center gap-1 sm:gap-2 shrink-0">
              ...
              <div
                ...
                className="hidden relative lg:flex items-center shrink-0"
              >
                <Link
                  href="/checkout/cart"
                  className="relative block"
                >
                  <div className="p-1.5 sm:p-2 rounded-full hover:bg-neutral-100 transition-colors">
                    <MdShoppingCartCheckout className="w-[21px] h-[21px] sm:w-[24px] sm:h-[24px] text-neutral-700" />
                  </div>

                  <span className="absolute -top-1 -right-1 flex items-center justify-center h-4 w-4 sm:h-5 sm:w-5 rounded-full bg-red-500 text-[9px] sm:text-[11px] font-bold text-white shadow-sm">
        ...
        {/* CATEGORY MENU */}
        <div
          className="relative mt-3 sm:mt-4 px-3 sm:px-4 lg:px-6"
        ...
        >
          ...
          <button
            ...
            className="hidden lg:flex items-center gap-2 cursor-pointer py-2 text-neutral-800 hover:text-red-600 transition-colors"
          >
          ...
          {isOpen && (
            <div
              ...
              className="absolute top-[calc(100%+4px)] left-3 sm:left-4 z-[80] flex w-auto lg:w-[750px] lg:h-[350px] bg-white rounded-xl shadow-2xl overflow-hidden border border-neutral-100"
            >
              {/* MAIN CATEGORIES */}
              <nav className="flex flex-col w-[210px] lg:w-[220px] shrink-0 overflow-y-auto border-r border-neutral-100 py-2 bg-neutral-50">
                {mainDataArray?.map((c) => {
              ...
              {/* SUB CATEGORIES */}
              <div className="flex-1 overflow-y-auto px-5 lg:px-6 py-5 bg-white relative">
                {isSubLoading ? (
                  ...
                  <>
                    ...
                    <div className="grid grid-cols-2 lg:grid-cols-3 gap-5 lg:gap-6">
                      {finalSubList?.map((col, idx) => (
                        <div key={col.id || idx} className="flex flex-col min-w-0">
                          <Link
                            href={`/search/${
                              col.slug || "category"
                            }?category_id=${col.id}`}
                          ...
                          >
                            ...
                            <TbChevronRight className="w-3.5 h-3.5 shrink-0 text-neutral-400 group-hover:text-red-600 transition-colors" />
                          </Link>
                          ...
                          {(col.children || col.subs || col.leaves || []).map(
                            (leaf, lIdx) => (
                              <Link
                                ...
                                className="py-1 text-[13px] text-neutral-500 hover:text-red-600 transition-colors truncate"
                              >
                                {leaf.name}
                              </Link>
                            )
          ...
          {isOpen && (
            <div className="md:hidden absolute top-[calc(100%+4px)] left-3 right-3 z-[80] bg-white rounded-xl shadow-2xl border border-neutral-100 overflow-hidden">
              <div className="max-h-[70vh] overflow-y-auto">
                {mainDataArray?.map((c) => (
                  <Link
                    ...
                    className="flex items-center justify-between px-4 py-3 border-b border-neutral-100 text-sm font-medium text-neutral-800 hover:bg-neutral-50 active:bg-neutral-100"
                  >
                    ...
                    <TbChevronRight className="w-4 h-4 text-neutral-400" />
                  </Link>
                ))}
              </div>
            </div>
